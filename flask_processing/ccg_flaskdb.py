...
class Flasks():
    ...
    def __init__(self, event, gas, database=None, debug=False):
        ...
        if database is None: database = "ccgg"
...
#        else:
#            self.sql.where("d.event_num=%s", events)
    ...
    #----------------------------------------------------------------------------------------------
    def updateDb(self, result, uncert, system, verbose=False):
        """ Update the database with results.

        Args:
            result: namedtuple with results to update. Members are
                (event, gas, mf, tagnum, inst, adate, ddate, comment)
            uncert: Uncertainty value to insert into database field.
