...
FUNCTION MY_CCG_SURFACE_SUBSET, $
   infile=infile, $
   saveas=saveas, $
   sp=sp, $
   lat=lat, $
...
print, "-------------"
   ...
   pl1 = j[N_ELEMENTS(j)-1]	; index where sinelat < latmin
   pl2 = k[0] NE -1 ? k[0] : pl1  ; index where sinelat > latmin
...
print, subset.sinelat
print, subset.data.dd
