...
def read_insitu_data_files(stacode, files):
    ...
    # don't assume all files have same format
    df = None
    data = []
    stdv = 0.0
...
#        print(len(data))
        ...
        df = pd.DataFrame(data, columns=['date_time', 'value', 'stdv', 'modenum', 'sample'])
...
def read_insitu_qc(files):
    ...
    for filename in files:
        if os.path.exists(filename):
            ds = pd.read_csv(filename, sep=r'\s+', names=['date', 'time', 'value'])
            ds['date_time'] = pd.to_datetime(ds.pop('date') + 'T' + ds.pop('time'))
            if df is None:
                df = ds
            else:
