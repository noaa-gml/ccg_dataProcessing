...
class dataWindow(wx.Panel):
    ...
    def __init__(self, parent, statusbar):
        ...
        self.plot.SetLocation(90, -150, 20, -40)
        self.plot.syncAxis(300)
        sizer1.Add(self.plot, 1, wx.EXPAND, 5)

    # ----------------------------------------------
    def setOptions(self, df, data, default=0):
        ...
        for name in self.df.columns:
            ...
            if "date" in name.lower(): continue
            val = self.df[name].iloc[0]
            if not isinstance(val, str):
                choices.append(title)

        choices.append("Reference Gas")
        choices.append("Response Curve")
        choices.append("Residuals")
        self.params["Reference Gas"] = "reference_gas"
        self.params["Response Curve"] = "response_curve"
    ...
    def updateParams(self, event=None):
        ...
        elif title == "Reference Gas":
            sysdata.reference(self.data, self.df, title, self.plot)
