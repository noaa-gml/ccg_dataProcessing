#!/usr/bin/env python
"""Query tool for ccgg database (flask and pfp measurements).
call from command line with -h for help.
This is python2/3 compatable 
"""
...
class CCGFlask(object) :
    def __init__(self, kwargs):
        super(CCGFlask, self).__init__()
        self.dev=False
        self.version=1.6
        self.args={}
        ...
        if self.args['verbose']>0 :
            print(("ccg_flask2.py version: " + str(self.version)))
    ...
    def bldBaseQuery(self):
        ...
        #search on tag
        #./ccg_flask2.py --not_tag_num=10 -s=sgp --from_ev_date=20210801
        if args.get('tag_num'):
    ...
    def parseArgs( self, kwargs ):
        ...
        descText="ccg_flask2.py version: "+str(self.version)+"""
------------------
...
from ccg_flask2 import ccg_flask
...
a=ccg_flask(args) #output written to out.dat in current working directory
...
###########
/ccg/src/db/ccg_flask2.py --cols='eventCols,narrMetCols,era5MetCols' --site=CAR --from_ev_date=2004-01-01 --to_ev_date=2005-01-01
    ->event details + narr met cols for car, samples in 2004

/ccg/src/db/ccg_flask2.py --site=CAR,alt --from_ev_date=2004-02-02 -g=co2,ch4 --flag=..%
    -> car and alt, from date, 2 gases, non rejected, standard columns

/ccg/src/db/ccg_flask2.py --inst=pr1 --cols=event_num --from_a_date='2016-01-01'
    -> all event #'s with PR1 measurements since 2016

/ccg/src/db/ccg_flask2.py --site='car' -g=co2 --from_ev_dd=2004.0 --to_ev_dd=2004.6 --to_alt=6000 --outfile='/tmp/out.csv'
    -> car measurements from first half of 2004, under alt:6000, send to csv file

/ccg/src/db/ccg_flask2.py --site=NWR --psuedo_pair_avg=1 -g=co2,co2c14 --flag='.%%'
    -> nwr co2,co2c14 with actual and psuedo pairs averaged together.

/ccg/src/db/ccg_flask2.py --site=car --program=hats --from_ev_date='2015-01-01' --cols='eventCols,dataCols,tag_nums' --tagDictionaryOutFile=/tmp/tags.txt --flag='.%' --not_flag
    ->reject data from 2015 forward for hats CAR data, write tag defs to file.

/ccg/src/db/ccg_flask2.py --site=hip --to_ev_date=2010-01-01 --output=map -g=co2
    ->show hippo samples thru 2010 on a map

/ccg/src/db/ccg_flask2.py --from_a_date='2019-05-01' --to_a_date='2019-09-01' --cols='event_num,dataCols,analysisCols' -g='co2'
...
/ccg/src/db/ccg_flask2.py --from_lat=50 --cols=eventCols,footprintCols -g=co2,ocs,co2c13 --outfile=/home/ccg/mund/tmp/t.csv
...
if __name__ == "__main__":
    ...
    r = ccg_flask(t)#call the wrapper function
