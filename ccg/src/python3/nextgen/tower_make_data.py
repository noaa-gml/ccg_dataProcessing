...
data = tower_read_rawdata.tower_read_rawdata(stacode, options.inst, date, qcconfig, verbose=options.verbose)
if 'value' not in list(data.keys()):
    if options.verbose:
...
# create an empty dataframe with just the index
df = pd.DataFrame(index=dates)

# add series to dataframe
for key in data:
    s = pd.Series(data[key][1], index=data[key][0])
    df[key] = s  # this will throw away rows that don't match a value in df.index (dates)


# convert the sysmodes number to syscode string
df['sample'] = tower_assign_syscode(df, config)
df['modenum'] = 1
...
if options.update:
    ...
    ccg_insitu_qc.write_insitu_data(outfile, df)
