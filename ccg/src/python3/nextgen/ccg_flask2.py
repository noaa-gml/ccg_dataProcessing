...
- Update Mar 2025:
    Handle measurement uncertainty for each of the calculation methods,
    and keep this value for including in the database.
    Moved total uncertainty calculations to the init() method after results
    are computed, instead of in the updateDB() method.
...
class Flask(ccg_process.processData):
    ...
    def __init__(self, rawfile, peaktype=None, scale=None, moddate=None, database=None, nocorr=False, debug=False):
        ...
        self.flaskdb = ccg_flaskdb2.Flasks(events, self.species, database=database, debug=debug)
        ...
        self.corrections = ccg_flask_corr.flaskCorrections(correction_file,
                                                           self.flaskdb.sample_data,
                                                           self.analyzer_id,
                                                           self.adate,
                                                           debug=debug)
        ...
        results = self._compute_mr()
    #    print self.results

        # compute total uncertainty ------------------
        # we're going to add in a total uncertainty value to the results

        # create a namedtuple object for storing results
        self.Result = namedtuple('row', ['event', 'gas', 'mf', 'unc', 'meas_unc', 'tag', 'inst', 'adate', 'comment'])

        # Read in the uncertainty data.  added 31 Jul 2018
        uncfile = "/ccg/flask/unc_master_test.conf"
#        uncfile = "/ccg/flask/unc_master.conf"
        uncert = ccg_uncdata_all.dataUnc('flask', self.species, self.site, self.system, uncfile=uncfile, debug=self.debug)

        self.results = []
        for t in results:
            (event, species, mf, meas_unc, tag, inst, adate, comment) = t
            sampleinfo = self._find_sample_data(event)
            if sampleinfo is None:
                ...
                unc = uncert.getUncertainty(sitecode, adate, mf, method, inst)
                if unc >= 0 and meas_unc >= 0:
                    unc = math.sqrt(unc*unc + meas_unc*meas_unc)

            t2 = (event, species, mf, unc, meas_unc, tag, inst, adate, comment)
            self.results.append(self.Result._make(t2))

    # ----------------------------------------------------------------------------------------------
    def _find_sample_data(self, event):
        """ Given an event number, return the flask sample information for that event. """
    ...
    def _compute_mr_co2_1(self):
        ...
        if self.debug:
            print("Using CO2 method 1, single flask bracketed by standards")

        # assigned values of reference tanks
        y = [t[1] for key, t in sorted(self.refgas.items())]
        ysd = [t[2] for key, t in sorted(self.refgas.items())]
#        print(y)
#        print(ysd)
        ...
        for i in self.raw.sampleIndices():
            ...
            if row.flag != ".":
                ...
                xsd = [vsd[key] for key in sorted(vsd)]
#                print(v)
#                print(x)
#                print(xsd)
            ...
            t = (row.event, self.species, round(mf, 2), unc, tag, self.analyzer_id, row.date, comment)
            results.append(t)
    ...
    def _compute_mr_co2_2(self):
        ...
        if self.debug:
            print("Using CO2 method 2, multiple flasks bracketed by standards")
...
#        y = [self.getRefgasMR(refid) for refid in gaslist]
        ...
        for i in range(self.raw.numrows):
            ...
            if row.smptype != "SMP":
                ...
                if self.debug:
                    print("Working on event ", row.smptype, row.event)
                ...
                if row.flag != ".":
                    ...
                    for refid in gaslist:
                        std1, unc1, valid1, idx1 = self._check_std(i, refid, prev=True)
                        std2, unc2, valid2, idx2 = self._check_std(i, refid, prev=False)

                        if self.debug:
                            print("   prev, next ref values:", valid1, valid2, std1, std2)

                        if self.nstds == 2:
                            # need to drift correct instead of average the bracketing refs
                            avgref = self.getDriftCorrValue(i, idx1, idx2, std1, std2, valid1, valid2)
                            uncref = (unc1 + unc2) / 2
                            tag, comment = TAGS['ok']

                        else:
                            (avgref, uncref, tag, comment) = self._get_avg_value(std1, std2, unc1, unc2, valid1, valid2)
                    ...
                    if DEFAULTVAL in y:
                        ...
                        # if no variance in the measured references, then use poly fit instead of odr
                        # odr can not work with 0 weighting for x axis values
                        if None in xsd:
                            coeffs = numpy.polyfit(x, y, deg=self.nstds-1)
                            mf = numpy.polyval(coeffs, row.value)
                            unc = -999.99

                        else:
                            print("   x values for odr:", x, xsd)
                            print("   y values for odr:", y, ysd)
                            fit, rsd = ccg_utils.odr_fit(self.nstds-1, x, y, xsd, ysd, self.debug)
                            print("   rsd:", rsd)
                            mf, unc = ccg_utils.odr_fit_val(self.nstds-1, row.value, fit, rsd)

                        tag, comment = TAGS['ok']

                    c = self._get_corr(mf, row.event, row.date)
                    mf += c
            ...
            for event_num, w in group.items():  # each dict has event num, indices
                ...
                if event_num.isnumeric():
                    ...
                    if len(vals) > 1 and smpdata['code'] != 'TST':
                        (avgmr, unc, tag, comment) = self._average_aliquots(vals)
                        # t elements are ('event', 'gas', 'mf', 'unc', 'tag', 'inst', 'adate', 'comment')
                        t = list(t)
                        t[2] = avgmr
                        t[3] = unc
                        t[4] = tag
    ...
    def _compute_mr_gc(self):
        ...
        for i in self.raw.sampleIndices():
            ...
            if row.flag != ".":
                ...
                if self.debug:
                    print("baseline code %s not in %s" % (row.bc, bcodes))
                mr = -999.99
                tag, comment = TAGS['baseline']

            else:
                # look for preceeding and following refs
                std1, unc1, valid1, idx1 = self._check_std(i, self.raw.refid, prev=True, bcodes=bcodes)
                std2, unc2, valid2, idx2 = self._check_std(i, self.raw.refid, prev=False, bcodes=bcodes)
                if self.debug:
                    print("   prev, next ref values:", valid1, valid2, std1, std2)
                ...
                mr, unc, tag, comment = self._get_mole_fraction(val, avgref, tag, comment)
            ...
            t = (row.event, self.species, round(mr, 2), unc_m, tag, self.analyzer_id, row.date, comment)
            results.append(t)
    ...
    def _compute_mr_laser(self):
        ...
        for i in self.raw.sampleIndices():
            ...
            if row.flag != ".":
                ...
                # look for preceeding and following refs
                std1, unc1, valid1, idx1 = self._check_std(i, self.raw.refid, prev=True, oneback=True)
                std2, unc2, valid2, idx2 = self._check_std(i, self.raw.refid, prev=False, oneback=True)
                if self.debug:
                    print("    prev, next ref values:", valid1, valid2, std1, std2)

                # determine standard deviation of the mean
                rowstd = row.std / math.sqrt(row.n)
#                print(std1, unc1, std2, unc2, rowstd)
                ...
                mr, unc, tag, comment = self._get_mole_fraction(row.value, avgref, tag, comment)
                c = self._get_corr(mr, row.event, row.date)
                mr = mr + c

                unc2 = self._get_meas_unc(row.value, rowstd, avgref, uncref)
                unc_m = math.sqrt(unc*unc + unc2*unc2)
#                print(mr, unc, unc2, unc_m)
                if self.debug:
                    print("    Measurement uncertainty", unc_m)
    ...
    def _compute_mr_vurf(self):
        ...
        for i in self.raw.sampleIndices():
            ...
            if row.flag != ".":
                ...
                (smpl, smpl_unc, flag) = self.raw.zeroCorrectSignal(i)  # returns zero corrected signal
                if self.debug:
                    print("   zero corrected signal is:", smpl)

                std1, unc1, valid1, idx1 = self._check_std(i, self.raw.refid, prev=True)
                std2, unc2, valid2, idx2 = self._check_std(i, self.raw.refid, prev=False)

                if self.debug:
                    print("   prev, next ref values:", valid1, valid2, std1, std2)

                (avgref, uncref, tag, comment) = self._get_avg_value(std1, std2, unc1, unc2, valid1, valid2)
                mr, unc, tag, comment = self._get_mole_fraction(smpl, avgref, tag, comment)
    ...
    def _get_mole_fraction(self, sample_val, avgstd, tag, comment):
        ...
        if avgstd in (0, DEFAULTVAL):
            ...
            if self.useResponseCurves:
                ...
                if conc == DEFAULTVAL:
                    if self.debug:
                        print("No value for reference %s." % key)
                    tag, comment = TAGS['norefvalue']
        ...
        if self.debug:
            ...
            print("    Value = ", value)
            print("    MR = %f" % (value))
            print("    Unc from response curve = ", unc)
    ...
    def printTable(self):
        ...
        format1 = "%10s %4s %s %13s %4s %10.2f %8.2f %8.2f %4s %5s       %s %s"
        ...
        print()
        print("   Event   Site    Sample Date         ID       Meth  Mole Fract    Unc     Munc   Tag  Inst       Analysis Date")
        print("----------------------------------------------------------------------------------------------------------------")
        ...
        for row in self.results:
            ...
            if line is None:
                ...
                print(format1 % (row.event, line['code'], line['date'].strftime("%Y-%m-%d %H:%M"),
                                 line['flaskid'], line['method'], row.mf, row.unc, row.meas_unc, row.tag, row.inst,
    ...
    def printResults(self, addalt=False, dbflags=False):
        ...
        format1 = "%10s %3s %s %13s %1s %8.2f %8.3f %8.3f %3s %3s %s"
        ...
        for row in self.results:
            ...
            if line is None:
                ...
                print(format1 % (row.event, line['code'], line['date'].strftime("%Y %m %d %H %M"),
                                 line['flaskid'], line['method'], row.mf, row.unc, row.meas_unc, flag, row.inst,
    ...
    def updateDb(self, verbose=False):
        ...
        for t in self.results:
            self.flaskdb.updateDb(t, self.system, verbose=verbose)
...
#            print("stats for", self.raw.method, "go here")
    ...
    #       print "\n\n";
            ...
            print(fmt % (self.raw.filename, self.adate.strftime("%Y %m %d %H %M %S"),
                         minratio, maxratio, conc, mean, sd, pr))
