...
# using response curve for NDIR systems.
...
#            rsd = self.coeffs[prev_cal_date][1] + drift * smp_tdiff
    ...
    def _find_cals(self, df):
        ...
        for row in df2.itertuples():
            ...
            if self.debug:
                print("index of first std", row.Index)
...
#            print(ysd)
    ...
    def _check_cal(self, start_cal, df, std_labels):
        ...
        # make sure we have all s1,s1,s2,s2,s3,s3 measurements with good flags

        if start_cal + 5 >= len(df): return False

        if df.loc[start_cal+1].label != std_labels[0]: return False
        if df.loc[start_cal+3].label != std_labels[1]: return False
        if df.loc[start_cal+5].label != std_labels[2]: return False
        if df.loc[start_cal+1].flag != '.': return False
        if df.loc[start_cal+3].flag != '.': return False
        if df.loc[start_cal+5].flag != '.': return False
    ...
    def _get_cal_diffs(self, start_cal, df, std_labels):
        ...
        for i in range(start_cal, start_cal+nstds*2, 2):
            row1 = df.loc[i]
            row2 = df.loc[i+1]
#            print(row1, row2)
            v = (row1.n * row1.value + row2.n * row2.value) / (row1.n + row2.n)
            std_values.append(v)

            v = (row1.reference + row2.reference) / 2
            ref_values.append(v)

            v = math.sqrt(row1.stdv**2 + row2.stdv**2)
            sig_values.append(v)

            v = math.sqrt(row1.ref_unc**2 + row2.ref_unc**2)
            ref_sig_values.append(v)


        # calculate difference from drift corrected R0 for each std.
        r = (numpy.array(std_values) - numpy.array(ref_values)).tolist()
        sigma = numpy.sqrt(numpy.array(sig_values)**2 + numpy.array(ref_sig_values)**2).tolist()
...
#        max_diff_resp = 4 * 60 * 60   # two days in seconds
        ...
        if len(df) == 1:
            ...
            # extrapolated values will be the same as the end points
            fsig = interpolate.interp1d(times, df.stdv.values, bounds_error=False, fill_value=(df.stdv.values[0], df.stdv.values[-1]))
