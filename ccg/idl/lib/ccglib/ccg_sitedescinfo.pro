PRO CCG_SITEDESCINFO, site=site,$
         proj_abbr=proj_abbr,$
         title=title,$
         position=position,$
         sample_ht=sample_ht,$
...
sql='SELECT name FROM ccgg.project WHERE abbr="'+proj_abbr+'"'
SPAWN, perlcode+" '"+sql+"'", proj_name
;
; Read MySQL CCG db
;
perlcode=dbdir+'ccg_sitedescinfo.pl'
...
IF ( N_ELEMENTS(tmp) EQ 15 ) THEN BEGIN
   binby = tmp[12]
   bmin=FLOAT(tmp[13])
   bmax=FLOAT(tmp[14])
...
;
; If the latitude is -99.9999 (default) then display "Variable Lat" in the
;    title
;
...
;
; If the longitude is -999.9999 (default) then display "Variable Lon" in the
;    title
;
...
;
; If the altitude is -9999.99 (default) then display "Variable Alt" in the
;    title
;
...
IF binby EQ 'alt' THEN BEGIN
   sample_str=STRCOMPRESS(STRING(FIX(bmin)),/RE)+' - '+STRCOMPRESS(STRING(FIX(bmax)),/RE)+' masl'
ENDIF ELSE BEGIN
   IF ( elev EQ -9999.99 OR intake EQ -9999.9 ) THEN BEGIN
...
CASE proj_abbr OF
   'ccg_aircraft': sample_ht=proj_name+' (Sample Altitude: '+sample_str+')'
   ELSE: sample_ht=proj_name+' (Sample Intake Height: '+sample_str+')'
ENDCASE
...
IF N_ELEMENTS(tmp) EQ 14 THEN BEGIN
   ...
   'alt': BEGIN
      ...
      ENDIF ELSE BEGIN
         ...
         z_str = z+'masl'
      ENDELSE
      sample_ht=proj_name+' (Sample Intake Height: '+z+')'
      END
   ENDCASE
ENDIF
END
