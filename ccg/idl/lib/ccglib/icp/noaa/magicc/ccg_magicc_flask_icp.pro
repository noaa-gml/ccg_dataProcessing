...
;
; CALLING SEQUENCE:
; 	CCG_MAGICC_FLASK_ICP, strategy = 'flask', site= 'tst', $
...
;
; 	CCG_MAGICC_FLASK_ICP, strategy = 'flask', sp='co2', $
...
;
; 	CCG_MAGICC_FLASK_ICP, project = 'ccg_aircraft', strategy = 'pfp', site='rta', $
;			sp = 'co2', date = [2006,2007], inst = 'l3,l8', $
;			/norm_inst1
;
;
; INPUTS:
;	project:	Project abbreviation.  May only specify one project. Not required.
...
;                       (ex) sp='co2' sp='n2o' sp='co2c13' sp='co2o18'
;
...
;
;       Discovered a typo in difference summary. 
;       The label "FLASKS RUN FIRST ON L3 (or L8) was
;       associated the opposite instrument.  The computation
;       and plot were correct, the summary labeling was incorrect.
;       Modified, AMC and KAM, September 5, 2008
;-
;
...
;       SAVE_FLASK_WRKDATA_FILES
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
PRO	SAVE_FLASK_WRKDATA_FILES, $
		wrkdata	= wrkdata, $
		ddir 	= ddir, $
		site 	= site, $
		ninst 	= ninst, $
...
; If site is 'tst', find cyln_values values and write test gas calibration
...
FOR i=0,ninst-1 DO BEGIN
	...
	format = '(A12,A5,A12,A8,A5,A12,A5,A12,A5,A15,A15)'
	...
	FOR j=0,nvalues-1 DO BEGIN
		...
		;PRINTF, fp1, FORMAT = '(I12,A5,G12.8,A8,A4,G12.8,I5,F12.4,A5,F12.4)', $
		PRINTF, fp1, FORMAT = '(I12,A5,G12.8,A8,A4,G12.8,I5,F12.4,A5,A15)', $
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; PROCEDURE TO WRITE DATA TO FILES ORGANIZED BY EVN FOR ARCHIVING
;       SAVE_EVN_FILES
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
PRO	SAVE_EVN_FILE, $
		wrkdata	= wrkdata, $
...
FOR i=0,ninst-1 DO BEGIN
	;tmp = wrkdata.(i).value
	IF site EQ 'TST' THEN tmp = [wrkdata.(i).value, wrkdata.(i).tg] ELSE tmp = wrkdata.(i).value
...
j = WHERE(data - 1.0 GT default)
ymin = (j[0] EQ -1) ? default : MIN(data[j]) * expand_ymin
ymax = (j[0] EQ -1) ? ABS(default) : MAX(data[j]) * expand_ymax
...
;if site=tst, plot cyln_values values as line
...
PRO	SPECIAL_TST_STATS, $
	...
	ntanks = N_ELEMENTS(cyln_values.start_date)
	first_index = MAX(WHERE(cyln_values.start_date LE LongDate2Dec(date[0])))	
	last_index = MIN(WHERE(cyln_values.start_date GT LongDate2Dec(date[1]))) - 1
	...
	FOR i = last_index, first_index, -1  DO BEGIN
		...
		CCG_DEC2DATE, cyln_values[i].start_date, tmpyr, tmpmo, tmpdy
		...
		IF i EQ ntanks-1 THEN BEGIN
			...
			k = WHERE( time_set GE cyln_values[i].start_date AND $
		...
		ENDIF ELSE BEGIN  
			...
			k = WHERE( time_set GE cyln_values[i].start_date AND $
				   time_set LT cyln_values[i+1].start_date  AND $
				   data_set -1 GT default)
			CCG_DEC2DATE, cyln_values[i+1].start_date - 0.00274, tmpyr, tmpmo, tmpdy
			tmpmo = STRING(tmpmo, FORMAT = '(I02)')
			tmpdy = STRING(tmpdy, FORMAT = '(I02)')
		...
		PRINTF, fp, FORMAT = fmt, tg_date_range, cyln_values[i].tank, avg, std, number
...
k = WHERE(diff - 1 GT default)
max_diff = (k[0] EQ -1) ? ABS(default) : MAX(ABS(diff[k]))
time = (k[0] EQ -1) ? [ABS(default),ABS(default)] : time[k]
...
FOR i=0,ninst-1 DO BEGIN
	...
	; if no valid data then continue
...
ncylinders = N_ELEMENTS(cyln_values.start_date)
plot_range = !X.CRANGE
FOR i=0, ncylinders-1 DO BEGIN
	IF cyln_values(i).start_date LE plot_range(0) OR cyln_values(i).start_date GE plot_range(1) THEN CONTINUE
	x = [cyln_values(i).start_date, cyln_values(i).start_date]
	y = [-10000.0, 10000.0]
...
max_diff = (k[0] EQ -1) ? ABS(default) : MAX(ABS(delta[k]))
...
FOR i = 0, ninst-2 DO BEGIN
	...
	; plot with filled symbols, flasks run first on inst(0) 
	...
	; plot with open symbols, flasks run first on inst(n) 
	IF N_ELEMENTS(l) EQ 1 THEN l = [l,l]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; PROCEDURE TO USE CCGVU TO FIT A CURVE TO THE DATA FROM EACH INSTRUMENT
;	CCG_ICP_CCGVU
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
PRO     CCG_ICP_CCGVU, $
                site = site, $
...
;  extract flask or pfp data from database using CCG_FLASK
...
IF (SIZE(tmpdata, /TYPE) NE 8) THEN BEGIN
        PRINT,'**************  NO DATA RETURNED FROM CCG_FLASK **************'

ENDIF ELSE BEGIN

	; Read lookup table for test gas tank assigned value
	 IF site EQ 'TST' THEN CYLINDER_VALUES, sp = sp, $
	...
	FOR i = 0, N_ELEMENTS(tmpdata) - 1 DO BEGIN
		...
		IF site EQ 'TST' THEN BEGIN
			...
			tg_value = CYLINDER_VALUES_BY_DATE( tmpdata[i].date, cyln_values, default)
	...
	ENDIF ELSE BEGIN
		...
		; plot mixing ratios
		IF KEYWORD_SET(mixing_ratio) THEN PLOT_FLASK_MIXING_RATIOS, $
			site = site, $
			date = date, $
			sp = sp, $
			inst = inst_, $
		...
		;  Plot difference between inst_(i) and inst_(0) for each evn number.
                ;  Only make call to plotting routine if ninst is 2 or greater, otherwise
                ;  print statement saying plot not available.
		IF KEYWORD_SET(norm_inst1) THEN	BEGIN

			IF ( ninst GE 2) THEN BEGIN
				PLOT_INST_MINUS_INST1	,$
				site = site, $
				sp = sp, $
				date = date, $
			...
			ENDIF ELSE PRINT, "Results not found for multiple instruments, comparison plot unavailable"

		ENDIF
				
		; If keyword "save_data" specified then Write text file for each wrkdata structure
		IF KEYWORD_SET(save) THEN SAVE_FLASK_WRKDATA_FILES, $
				wrkdata = wrkdata, $
				ddir = ddir, $
				site = site, $
				ninst = ninst, $
