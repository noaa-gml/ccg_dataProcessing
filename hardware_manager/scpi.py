...
class scpi(hmsubs.hmsub):
    ...
    def open_relay(self, device, option):
        ...
        # check relay status
        status = self.check_close_relay(device, option)
        self.lock_interface(device, "unlock")
        if int(status) == 0:
            self.logger.info("Relay Status:  %s %s %s -- %s", device.name, option, int(status), "OPEN")
        elif int(status) == 1:
            self.logger.info("Relay Status:  %s %s %s -- %s", device.name, option, int(status), "CLOSED")
    ...
    def read_number(self, device, option):
        ...
        self.logger.info("ReadNumber %s %s %f", device.name, option, val)
