...
require "/var/www/html/inc/date.php";

class Validator_Utils
{
   #
   ...
   {
      if ( $this->isBlank($d_val) ) { return FALSE; }
      ...
      if ( ! $this->ValidinRange($yr,$mindateyr,$maxdateyr,"int") ) { return FALSE; }
      if ( ! $this->ValidinRange($mo,$mindatemo,$maxdatemo,"int") ) { return FALSE; }
      if ( ! $this->ValidinRange($dy,$mindatedy,$maxdatedy,"int") ) { return FALSE; }
   ...
   {
      if ( $this->isBlank($dt_val) ) { return FALSE; }
   ...
   function ValidinRange($value, $min, $max, $type="other")
   {
      ...
      {
         case "date":
            if ( ! $this->ValidDate($value) ) { return FALSE; }
            ...
            if ( ! $this->isNULL($min) )
            {
               if ( ! $this->ValidDate($min) ) { return FALSE; }
            ...
            if ( ! $this->isNULL($max) )
            {
               if ( ! $this->ValidDate($max) ) { return FALSE; }
            ...
            break;
         case "int":
            if ( ! $this->ValidInt($value) ) { return FALSE; }

            if ( ! $this->isNULL($min) )
            {
               if ( ! $this->ValidInt($min) ) { return FALSE; }
               if ( intval($value) < intval($min) ) { return FALSE; }
            }

            if ( ! $this->isNULL($max) )
            {
               if ( ! $this->ValidInt($max) ) { return FALSE; }
               if ( intval($value) > intval($max) ) { return FALSE; }
            }
            break;
         case "float":
            if ( ! $this->ValidFloat($value) ) { return FALSE; }

            if ( ! $this->isNULL($min) )
            {
               if ( ! $this->ValidFloat($min) ) { return FALSE; }
               if ( floatval($value) < floatval($min) ) { return FALSE; }
            }

            if ( ! $this->isNULL($max) )
            {
               if ( ! $this->ValidFloat($max) ) { return FALSE; }
   ...
   {
      if ( $this->isBlank($t_val) ) { return FALSE; }
      ...
      if ( ! $this->ValidinRange($hr,$mintimehr,$maxtimehr,"int") ) { return FALSE; }
      if ( ! $this->ValidinRange($mn,$mintimemn,$maxtimemn,"int") ) { return FALSE; }
      if ( ! $this->ValidinRange($sc,$mintimesc,$maxtimesc,"int") ) { return FALSE; }
   ...
   {
      if ( ! $this->isLetter($val) && ! $this->isDigit($val) ) { return FALSE; }
      return TRUE;
   }
   ...
   {
      if ( ! $this->isLowerLetter($val) && ! $this->isUpperLetter($val) )
      { return FALSE; }
      return TRUE;
   }
