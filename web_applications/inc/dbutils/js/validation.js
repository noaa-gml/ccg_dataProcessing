	/*
     *jwm- 
     *Note dateval below hasen't tested (i prefixed all the sub functions below), but general code should be pretty solid.
     *5/17
     *
...
function validateDate(inputID,allowTime,coerceDir) {
    ...
    if (dateStr.length>0) {
        ...
        for(i=0;i<dateStr.length;i++){//Loop thru until hit a seporator or am/pm string
            ...
            if (vd_isSeporator(ch) || vd_isAMPM(ch)) {
        ...
        if (parts[0].length==8 && vd_isNumeric(parts[0]))parseFormat='yyyymmdd';
        if (parts[0].length==4 && vd_isNumeric(parts[0]))parseFormat='ymd';
        switch (parseFormat) {
            case "mdy":
                month=parts[0];
                day=parts[1];
        ...
        year=(vd_isNumeric(year))?year:"";
        month=(vd_isNumeric(month))?month:"";
        day=(vd_isNumeric(day))?day:"";
        ...
        if(allowTime){
            if(vd_isNumeric(parts[3])){//any time entered? note; time entries will start in parts[3] if present
                hour=parts[3];
                gotTime=true;
                minute=parts[4];
                second=parts[5];
                ampm=(vd_isAMPM(parts[6]))?parts[6]:"";
                //See if all time parts were entered and default 0 if not.
                if(vd_isAMPM(minute)){//Just hr passed
                    ampm=minute;
                    minute="0";
                    second="0";
                }
                ...
                hour=(vd_isNumeric(hour))?hour:"0";
                minute=(vd_isNumeric(minute))?minute:"0";
                second=(vd_isNumeric(second))?second:"0";
        ...
        if (validateDatePart(year,"year") && validateDatePart(month,"month") && validateDatePart(day,"day") && validateDatePart(hour,"hour") && validateDatePart(minute,"minute") && validateDatePart(second,"second") ) {
            ...
            if (d) {
                var str=vd_createDateString(d,"ymd");
                if (str!="") {
                    success=true;
                    if (gotTime) {
                        var tstr=vd_createTimeString(d,'24hms');
                        if (tstr!="") {
                            str=str+" "+tstr;
                        }
                    }
...
function vd_setFocus(inputName){//delay the focus to give everything a chance to run thru.  Had a nasty timing issue where tabbing out to next field occured after onChange script occured and we couldn't ever get the focus to stick.. This seems to solve the problme well.
	setTimeout(inputName+".focus()",100);
}
function vd_formatNumber(num){
	if(num=="")num="00";
...
function vd_isSeporator(ch){
	if(ch=="." ||ch=="," || ch=="-" || ch==":" || ch=="/" || ch==" ") return true;
	else return false;		
}
function vd_isAMPM(ch){
	ch=ch.substring(0,1);
	ch=ch.toUpperCase();
	if(ch=="P" || ch=="A") return true;
	else return false;
}
function vd_createTimeString(dDate,timeFormat){
	var ampm, h24;
...
function vd_createDateString(dDate,datePartOrder){
...
function vd_isNumeric(checkString){
...
function pairedInputs(val1, val2) {
    //Returns true if both are entered or neither.  False if one or the other.
    //Note this doesn't check for 'truthiness', just whether there is an entry.
