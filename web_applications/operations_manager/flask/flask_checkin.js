...
function FlaskSelection(id)
{
   ...
   if (availablelistindex < 0) { alert(id+' not found in available flask list.');return; }
   ...
   { oflask_notes.innerHTML = '** FLASK **<BR>['+id+'] '+flask_notes[availablelistindex]+'<P>'; }
...
function OkayCB()
{
   ...
   if (confirm('Are you sure?')) {f.task.value='Ok'; f.submit();}
   else { return; }
}
