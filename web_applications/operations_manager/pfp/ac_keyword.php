...
BuildNavigator();
echo "<SCRIPT language='JavaScript' src='ac_keyword.js'></SCRIPT>";
...
switch ($task)
{
   ...
   case 'update':
      ...
      #
      DB_UpdateInfo($table,$key_info,$selectkey,$task);
      $selectkey = '';
      $task = '';
      break;
}
...
#
$keyinfo = DB_GetAllKeyList($table);
...
function MainWorkArea()
{
   ...
   # is the currently selected keyword.
   # 
   ...
   echo "<TD align='center' class='XLargeBlueB'>Aircraft Keyword Manager</TD>";
   echo "</TR>";
   echo "</TABLE>";
   #
   ##############################
   ...
   echo "<FONT class='MediumBlackN'>Select Keyword:</FONT><BR>";
   echo "<SELECT class='MediumBlackN' NAME='keylist' SIZE='5' onChange='ListSelectCB(this)'>";

   $selected = ( $selectkey == "Add" ) ? "SELECTED" : "";
   echo "<OPTION $selected VALUE='Add'>Add Keyword</OPTION>";

   for ($i=0,$z=''; $i<count($keyinfo); $i++)
   {
      $tmp = split("\|", $keyinfo[$i]);
      $z = sprintf("%s",$tmp[1]);
      $selected = ( $tmp[0] == $selectkey ) ? "SELECTED" : "";
   ...
   #
   PostTable2Edit($table,$selectkey,'DESCRIPTION');
   ...
   #
   # By default this button will be add. If the user clicks on a 
   # existing keyword, then the button will change to "Update".
   # If user clicks on "Add Keyword", then we change this button to "Add".
   #
   echo "<TD align='center'>";
   echo "<INPUT TYPE='button' name='addupdate' class='Btn' value='Add' onClick='ExecuteCB()'>";
   echo "</TD>";
                                                                                          
   echo "<TD align='center'>";
   echo "<INPUT TYPE='button' class='Btn' value='Clear' onClick='ClearCB()'>";
   echo "</TD>";
...
#
function PostTable2Edit($table,$keynum,$title)
{
   global $keyinfo;
   ...
   #
   # Get the information about the keyword
   #
   ...
   for ($i=0; $i<count($name); $i++)
   {
      ...
      switch ($type[$i])
      {
         case "blob":
            echo "<TD ALIGN='left'>";
            echo "<TEXTAREA class='MediumBlackN' name='info:$name[$i]' cols=60 rows=5 onFocus='SetBackground(this,true)' onBlur='SetBackground(this,false)'>";
            echo $value;
            echo "</TEXTAREA></TD>";
            break;
         default:
            ...
            if ( $writable )
            {
               echo "<INPUT type=text class='MediumBlackN' name='info:$name[$i]' value='${value}' size='60' onFocus='SetBackground(this,true)' onBlur='SetBackground(this,false)'>";
            }
            else
            {
               echo "<INPUT type=text class='MediumBlackN' name='info:$name[$i]' value='${value}' size='60' onFocus='SetBackground(this,true)' onBlur='SetBackground(this,false)' style='background-color: #E6E6E6;' DISABLED>";
            }
	    break;
      }
      echo "</TR>";
...
#
function DB_GetAllKeyList($table)
{
   #
...
#
function DB_UpdateInfo($table,$info,$keynum,$settask)
{
   #
   ...
   if ( $keynum != 'Add' )
   {
      $sql = "SELECT COUNT(*) FROM ${table} WHERE num = $keynum";
   }
   else
   {
