...
echo "<TD align='center' class='XLargeBlueB'>Shipping History By PFP Id</TD>";
echo "</TR>";
echo "</TABLE>";
#
##############################
...
echo "<TD width='33%' align='left' class='MediumBlackB'>PFP Id - <FONT class='MediumBlueB'>${id}</FONT></TD>";
...
function DB_GetShipById($id,&$arr)
{
	...
	#
	$select = "SELECT pfp_shipping.id,gmd.site.code,project.abbr";
	$select = "${select},pfp_shipping.date_out,pfp_shipping.date_in";
	$select = "${select},TO_DAYS(pfp_shipping.date_in) - TO_DAYS(pfp_shipping.date_out) AS days";
	$from = " FROM pfp_shipping,gmd.site,project";
	$where = " WHERE pfp_shipping.id='${id}'";
	$and = " AND gmd.site.num=pfp_shipping.site_num";
        $and = "${and} AND project.num=pfp_shipping.project_num";
	$etc = " ORDER BY pfp_shipping.date_out DESC,site.code ASC LIMIT 400";
