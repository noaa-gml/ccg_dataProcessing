...
BuildNavigator();
echo "<SCRIPT language='JavaScript' src='pfp_shipbysite.js'></SCRIPT>";
...
if ($code) { $shipinfo = DB_GetShipBySite($code, $proj_abbr); }
...
echo "<TD>";
echo "<TEXTAREA class='MediumBlackMonoN' NAME='atsitelist' ROWS='15' COLS='46'>";
...
#
function DB_GetShipBySite($code, $proj_abbr)
{
   #
   # Get list of pfps checked out by site
   #
   $select = "SELECT gmd.site.code,pfp_shipping.id";
   ...
   $and = "${and} AND gmd.site.num=pfp_shipping.site_num";
   $and = "${and} AND project.num=pfp_shipping.project_num";
   $etc = " ORDER BY pfp_shipping.date_out DESC,pfp_shipping.id ASC LIMIT 400";
