...
/*
0.07 works for the MOXA IP-to-serial but not for the Digi-One SP
Try 0.12.  November 12, 2004 - kam
*/
/* changed from 0.12 to 1.0, January 4, 2005 - kam,dwg */
  ...
  char *p_string;
{
  ...
  // Replace the last character if it is a carriage return
  len = strlen(p_string);
    ...
    
{
  ...
  send_as_msg( as_eid, prompt_prompt );        /* Get PFP to kick out the menu */
  get_as_msg( as_eid, reply );                 /* to see if this is a 3.03 or */
  if (strstr( reply, "(T)ime" ) != NULL)       /* 3.06+; set the 'set time'*/
    date_cmd = "T\r\n";                        /* prompt accordingly. */
  else
    date_cmd = "D\r\n";

  send_as_msg(as_eid, date_cmd);
  get_as_msg(as_eid, reply);
  /* printf("%s\n",reply); */
  i=0;
     ...
     char codestr[3];
{
  ...
  send_as_msg( as_eid, prompt_prompt );        /* Get PFP to kick out the menu */
  get_as_msg( as_eid, reply );                 /* to see if this is a 3.03 or */
  if (strstr( reply, "(C)ode" ) != NULL)       /* 3.06+; set the 'set time'*/
    sitecode_cmd = "C\r\n";                        /* prompt accordingly. */
  else
    sitecode_cmd = "N\r\n";

  send_as_msg(as_eid, sitecode_cmd);
  get_as_msg(as_eid, reply);
  /*printf("%s\n", reply);*/
