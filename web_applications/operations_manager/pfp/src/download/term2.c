...
/* Revision History
  ...
  17 Jul 1996	try curses routines to eliminate double echoing of keyboard characters
*/
...
int open_as_comm()
{
  ...
  if ((eid = open( "/dev/tty09", (O_RDWR | O_NDELAY), 0 )) < 0)
  {
  ...
  {					            /* Create a termios block: */
    ...
    if (ioctl( eid, TCSETATTR, &termcon ) < 0)
    {
...
main()
{
  ...
  while (1)
  {
    if ((inchar = getch()) > 0) 		/* Use curses routines to get keyboard input. */
    {
    ...
    while (read( fdout, pin, 1 ) > 0)		/* Buffer incomming serial characters. */
      if (++pin > pend)
	pin = buff;				/* Wrap the buffer on input. */

    if (pin != pout) 
    {
      addch( *pout++ );				/* Use curses routine to put character to screen. */
      if (pout > pend)
