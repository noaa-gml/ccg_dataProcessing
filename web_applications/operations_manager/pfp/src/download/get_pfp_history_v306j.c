...
/* Revisions:
	...
	16 Mar 2009	changed revision name to v306j, still handles older PFP versions
			fix time offset read for version 3.06j
*/
...
/* Tragically, the date and time report is reversed in PFP version 3.03
  versus 3.06+. Hence an extra decision step here to get the order right. */
     ...
     first_time_value = get_next_timevalue( &p_buffy );
     if (first_time_value > 24)         /* Must be coming out year first from */
                                        /* a version 3.06j. */
     {
       pfp_datetime.tm_year = first_time_value - 1900;
     ...
     else                               /* Else must be coming out hour first */
                                        /* from a version 3.03. */
     {
       pfp_datetime.tm_hour = first_time_value;
       pfp_datetime.tm_min =  get_next_timevalue( &p_buffy );
       pfp_datetime.tm_sec =  get_next_timevalue( &p_buffy );
