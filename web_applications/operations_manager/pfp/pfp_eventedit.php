...
if ( empty($event_detail) ) { $ev_project = "ccg_aircraft"; }
else
{
   list($ev_code,$ev_project,$ev_id,$ev_date,$ev_time,$ev_meth,$ev_temp,$ev_press,
   $ev_rh,$ev_lat,$ev_lon,$ev_alt,$ev_comment,$ev_num,$lst2utc,$status_num,
...
BuildNavigator();
echo "<SCRIPT language='JavaScript' src='pfp_eventedit.js'></SCRIPT>";
...
switch ($task)
{
   case "search":
      ...
      { JavaScriptAlert("${search4event} does not exist as a PFP event number"); }
...
if ($ev_num != '0' && $ev_num != '')
{
   ...
   # June 2006 - kam, pml
   #
   if (strcmp($ev_time, "123456") == 0) { $ev_time = "999999"; }
...
echo "<INPUT TYPE='HIDDEN' NAME='last_temp' VALUE='${ev_temp}'>";
echo "<INPUT TYPE='HIDDEN' NAME='last_press' VALUE='${ev_press}'>";
echo "<INPUT TYPE='HIDDEN' NAME='last_rh' VALUE='${ev_rh}'>";
...
echo "<INPUT TYPE='HIDDEN' NAME='last_alt_source' VALUE='${ev_alt_source}'>";
...
echo "</TR>";
echo "<TR>";
echo "<TD align='right' class='MediumBlackN'> Time [014354]</TD>";
...
onFocus='SetBackground(this,true)' onBlur='SetBackground(this,false)' 
class='MediumBlackN' SIZE=10 MAXLENGTH=10></TD>";
echo "</TR>";
echo "<TR>";
echo "<TD align='right' class='MediumBlackN'> Method [A]</TD>";
echo "<TD><INPUT TYPE='text' NAME='ev_meth' VALUE='${ev_meth}'
onFocus='SetBackground(this,true)' onBlur='SetBackground(this,false)' 
class='MediumBlackN' SIZE=10 MAXLENGTH=1></TD>";
echo "</TR>";
echo "<TD align='right' class='MediumBlackN'> Temperature [-999.9]</TD>";
echo "<TD><INPUT TYPE='text' NAME='ev_temp' VALUE='${ev_temp}'
onFocus='SetBackground(this,true)' onBlur='SetBackground(this,false)' 
class='MediumBlackN' SIZE=10 MAXLENGTH=6></TD>";
echo "</TR>";

if ( empty($ev_temp) && !empty($ev_num) )
{ JavaScriptCommand("document.mainform.ev_temp.value = defaults1.temp"); }

echo "<TR>";
echo "<TD align='right' class='MediumBlackN'> Pressure [-9999.9]</TD>";
echo "<TD><INPUT TYPE='text' NAME='ev_press' VALUE='${ev_press}'
onFocus='SetBackground(this,true)' onBlur='SetBackground(this,false)' 
class='MediumBlackN' SIZE=10 MAXLENGTH=7></TD>";
echo "</TR>";

if ( empty($ev_press) && !empty($ev_num) )
{ JavaScriptCommand("document.mainform.ev_press.value = defaults1.press"); }

echo "<TR>";
echo "<TD align='right' class='MediumBlackN'> Relative Humidity [-999.9]</TD>";
echo "<TD><INPUT TYPE='text' NAME='ev_rh' VALUE='${ev_rh}'
onFocus='SetBackground(this,true)' onBlur='SetBackground(this,false)' 
class='MediumBlackN' SIZE=10 MAXLENGTH=6></TD>";
echo "</TR>";

if ( empty($ev_rh) && !empty($ev_num) )
{ JavaScriptCommand("document.mainform.ev_rh.value = defaults1.rh"); }
...
echo "</TR>";
echo "<TR>";
echo "<TD align='right' class='MediumBlackN'> Altitude Source</TD>";
echo "<TD><SELECT class='MediumBlackN' NAME='ev_alt_source'
SIZE='1'>";
$alt_sources['--'] = '';
$alt_sources['na'] = 'na';
$alt_sources['plan'] = 'plan';
$alt_sources['plan_edit'] = 'plan_edit';
$alt_sources['plan_met'] = 'plan_met';
$alt_sources['plan_met_edit'] = 'plan_met_edit';
$alt_sources['gps'] = 'gps';
$alt_sources['gps_edit'] = 'gps_edit';
$alt_sources['gps_nav'] = 'gps_nav';
$alt_sources['gps_nav_edit'] = 'gps_nav_edit';
$alt_sources['db'] = 'db';
$alt_sources['db_edit'] = 'db_edit';
$alt_sources['met'] = 'met';
$alt_sources['met_edit'] = 'met_edit';

foreach ( $alt_sources as $name => $value )
{
   $selected = ( $ev_alt_source == $value ) ? 'SELECTED' : '';
   echo "<OPTION VALUE='$value' $selected>$name</OPTION>";
}
echo "</SELECT>";
echo "</TD>";
echo "<TD>";
echo "<INPUT type='button' class='Btn' name='ev_alt_source_help' onClick='Help_AltSource()' value='?'>";
echo "</TD>";
echo "</TR>";
echo "<TR>";
...
if ($status_num != 0)
{
   ...
   if ($ev_num != '' && $status_num != '2' && $status_num != '4')
   {
      ...
      $dummy = str_repeat("-",30);
      #
      # Analysis path assigned to sample
      #
      $apath = split(",",$ev_path);
...
function DB_GetEventDetails($ev_num)
{
   ...
   $and = "${and} AND flask_event.strategy_num='2'";
   ...
   $from = " FROM pfp_inv";
   $where = " WHERE pfp_inv.event_num='${ev_num}'";
...
function DB_UpdateEventNum($event_detail)
{
   ...
   $ev_path) = split("\|",$event_detail);
   #
   # Reset Default time from 99:99:99 to 12:34:56
   # June 2006 - kam, pml
   #
   ...
   $set = "${set},project_num='${proj_num}',strategy_num='2',dd='${dd}'";
