...
BuildNavigator();
echo "<SCRIPT language='JavaScript' src='pfp_outbysite.js'></SCRIPT>";
...
if ($code) { $atsiteinfo = DB_GetPFPsOutBySite($code); }
...
echo "<TD align='center' class='XLargeBlueB'>PFPs Out By Site<BR>";
echo "<FONT class='MediumBlackN'>${nout} PFPs Checked Out (All Sites)</FONT></TD>";
echo "</TR>";
echo "</TABLE>";
#
##############################
...
echo "<TD width='22%' align='left' class='MediumBlackB'>PFPs - <FONT class='MediumBlueB'>${n}</FONT></TD>";
...
#
# Function DB_GetPFPsOut ########################################################
#
function DB_GetPFPsOut()
{
   #
   # Get list of PFPs checked out
   #
   $select = "SELECT pfp_inv.id,gmd.site.code,pfp_inv.date_out";
   $select = "${select}, project.abbr";
   $from = " FROM pfp_inv,gmd.site,project";
   $where = " WHERE pfp_inv.sample_status_num='2'";
   $and = " AND gmd.site.num=pfp_inv.site_num";
   $and = "${and} AND project.num=pfp_inv.project_num";
   $and = "${and} AND pfp_inv.id LIKE '%-FP'";

   return ccgg_query($select.$from.$where.$and);
}
#
# Function DB_GetPFPsOutBySite ########################################################
#
function DB_GetPFPsOutBySite($code)
{
   #
   # Get list of PFPs checked out by site
   #
   $select = "SELECT pfp_inv.id,gmd.site.code,pfp_inv.date_out";
   $select = "${select}, project.abbr";
   $from = " FROM pfp_inv,gmd.site,project";
   ...
   $and = " AND gmd.site.num=pfp_inv.site_num";
   $and = "${and} AND project.num=pfp_inv.project_num";
   $and = "${and} AND gmd.site.code='${code}'";
   $and = "${and} AND pfp_inv.id LIKE '%-FP'";
   $etc = " ORDER BY pfp_inv.date_out DESC";
