...
BuildNavigator();
echo "<SCRIPT language='JavaScript' src='gen_tlog_keyword.js'></SCRIPT>";

#
#    Set initial variables as well as global variables that were returned
# from the last submission
#
$table = "${ccgg_equip}.gen_tlog_keyword";
$info = $key_info;
$number = $selectkey;
...
function MainWorkArea()
{
   ...
   #
   $type = "${ccgg_equip}.gen_tlog_keyword";
...
function CreateTypeSelectButton($table)
{
   ...
   echo "<TD ALIGN='left'>";
   echo "<SELECT NAME='$table:gen_tlog_commenttype_num' class='MediumBlackN' SIZE=1>";
...
function DB_GetAllKeyList()
{
   ...
   #
   $select = " SELECT num, gen_tlog_commenttype_num, gen_type_num, name, comments";
...
function DB_GetAllKeyCount()
{
   ...
   #
   # Get a list of keywords from the gen_tlog_case table
   #
   $select = " SELECT t1.num, COUNT(*)";
   $from = " FROM ${ccgg_equip}.gen_tlog_keyword AS t1, ${ccgg_equip}.gen_tlog_case AS t2";
   $where = " WHERE t1.gen_tlog_commenttype_num = '1'";
   $and = " AND t2.gen_tlog_keyword_num = t1.num";
   $group = " GROUP BY t1.num";
   $order = " ORDER BY num";
   $sql = $select.$from.$where.$and.$group.$order;
   $tmparr = ccgg_query($sql);

   #
   # Get a list of keywords from the gen_tlog_comment table
   #
   $select = " SELECT t1.num, COUNT(*)";
   $from = " FROM ${ccgg_equip}.gen_tlog_keyword AS t1, ${ccgg_equip}.gen_tlog_comment AS t2";
   $where = " WHERE t1.gen_tlog_commenttype_num > '1'";
   $and = " AND t2.gen_tlog_keyword_num = t1.num";
   $group = " GROUP BY t1.num";;
   $order = " ORDER BY num";
   ...
   $outarr = array_merge($outarr, $tmparr);
