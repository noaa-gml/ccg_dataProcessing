...
function AvailableListCB(shownotes)
{
   var f = document.mainform;
   if ( f.availablelist.selectedIndex == -1 ) return;
   UnitSelection(f.availablelist[f.availablelist.selectedIndex].value);
...
function OkayCB()
{
   ...
   {
      f.id.value = f.selectedunits.value;
      f.action = 'https://omi.cmdl.noaa.gov/om/gen/gen_checkininfo.php?invtype='+f.invtype.value+'&strat_name='+f.strat_name.value+'&strat_abbr='+f.strat_abbr.value;;
      f.submit();
   }
   else { return; }
}
...
function UnitScanCB()
{
   ...
   UnitSelection(f.unitscan.value);
   f.unitscan.blur();
   f.unitscan.value = '';
   f.unitscan.focus();
}
