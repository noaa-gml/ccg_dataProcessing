...
BuildNavigator();
echo "<SCRIPT language='JavaScript' src='gen_dbquery.js'></SCRIPT>";
...
function DB_GetAllQueryList()
{
   ...
   $from = "FROM ${ccgg_equip}.gen_dbquery ";
...
function DB_UpdateInfo($newqueryinfo)
{
   ...
   $sql = "SELECT COUNT(*) FROM ${ccgg_equip}.gen_dbquery";
   ...
   if ( $task == 'update' )
   {
      if ( $name != '' && $command != '')
      {
         $sql = "UPDATE ${ccgg_equip}.gen_dbquery SET ${set} ";
         $sql = "${sql} WHERE name='${name}' AND user='$user'";
         $sql = "${sql} AND gen_type_num = '${gen_type_num}'";
   ...
   if ( $task == 'add' )
   {
      if ( $name != '' && $command != '')
      {
         $sql = "INSERT INTO ${ccgg_equip}.gen_dbquery";
         $sql = "${sql} VALUES ('$gen_type_num','$user','${name}','$date','$time','${command}','${comments}')";
...
function DB_DeleteInfo($newqueryinfo)
{
   ...
   $sql = "DELETE FROM ${ccgg_equip}.gen_dbquery";
   $sql = "${sql} WHERE name ='${name}' AND user='$user'";
   $sql = "${sql} AND gen_type_num = '${gen_type_num}'";
   $sql = "${sql} LIMIT 1";
