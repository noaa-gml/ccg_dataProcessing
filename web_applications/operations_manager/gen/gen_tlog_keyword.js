   ...
   else
   {
      ...
      if ( f.prev_selectkey.value != '' )
      {
         for ( i=0; i<arr.length; i++ )
         {
            ...
            if ( tmp[0] == f.prev_selectkey.value )
            {
               values = tmp;
               break;
            }
      ...
      for (ii=0; ii<f.elements.length; ii++)
      {
         ...
         if ( names[1] == tmp[0] )
         {
            ...
            if ( configchk )
            {
               ...
               //
               if ( names[1] == 'gen_tlog_commenttype_num' || names[1] == 'name')
               {
...
function FillSelect(typenum)
{
   ...
   for ( i=0; i<arr.length; i++ )
   {
      ...
      if ( tmp[1] == typenum )
      {
         f.keylist[count] = new Option(tmp[3],tmp[0],false,false);
