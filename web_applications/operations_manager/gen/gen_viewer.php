...
BuildNavigator();
echo "<SCRIPT language='JavaScript' src='/om/flask/flask_viewer.js'></SCRIPT>";
...
else
{
   if ( empty($searchstr) )
   {
      ...
      {
         case 'in_testing':
            $arr = DB_GetUnitsInTesting();
            $count = count($arr);
            $title = "${gen_type_abbr}s in Testing";
            ...
            break;
         case 'available':
            $arr = DB_GetAvailUnits();
            $count = count($arr);
            $title = "${gen_type_abbr}s Available for Checkout";
            ...
            break;
         case 'in_repair':
            $arr = DB_GetUnitsInRepair();
            $count = count($arr);
            $title = "${gen_type_abbr}s in Repair";
            ...
            break;
         case 'retired':
            $arr = DB_GetUnitsRetired();
            $count = count($arr);
            $title = "Retired ${gen_type_abbr}s";
            ...
            break;
         case 'notes':
            $arr = DB_GetUnitsWithNotes();
            $count = count($arr);
            $title = "${gen_type_abbr}s with Notes";
...
function DB_GetUnitsInTesting()
{
   ...
   #
   # Get list of units that are in testing
   #
   $select = " SELECT id";
...
function DB_GetUnitsInRepair()
{
   ...
   #
   # Get list of units that are in repair
   #
   $select = " SELECT id, date_out";
   $from = " FROM ${ccgg_equip}.gen_inv";
   $where = " WHERE gen_status_num = '4'";
   $and = " AND gen_type_num = '${gen_type_num}'";

   return ccgg_query($select.$from.$where.$and);
}
#
# Function DB_GetUnitsRetired ########################################################
#
function DB_GetUnitsRetired()
{
   global $ccgg_equip;
   ...
   #
   # Get list of units that have been retired
   #
   $select = " SELECT id, date_out";
   $from = " FROM ${ccgg_equip}.gen_inv";
   $where = " WHERE gen_status_num = '5'";
   $and = " AND gen_type_num = '${gen_type_num}'";

   return ccgg_query($select.$from.$where);
}
#
# Function DB_GetUnitsWithNotes ########################################################
#
function DB_GetUnitsWithNotes()
{
   global $ccgg_equip;
