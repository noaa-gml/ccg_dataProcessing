...
class mkSignalPage(wx.Panel):
	...
	self.t2 = wx.Timer(self)
        ...
        box = wx.StaticBox(self, -1, "Historical Signals", size=(10,10))
	...
	if os.path.exists(file):
		...
		for linenum, line in enumerate(raw):
			...
			if gas.lower() == "lgr":
			    ...
			    elif type == "Gas_Temp":  v = float(list[23])
			    elif type == "Amb_Temp":  v = float(list[26])
			    elif type == "Flow":       v = float(list[29])
			    elif type == "Smpl_Press": v = float(list[30])
			    elif type == "Room_temp":  v = float(list[31])
			    elif type == "626_ppm":  v = float(list[32])
			    elif type == "636_ppm":  v = float(list[35])
			    elif type == "628_ppm":  v = float(list[38])
			    else:
				    pass
