...
{
fprintf(stderr, "INSIDE PEAK CANCEL\n");
...
#ifdef DEBUG
   fprintf(stderr, "peak crest %d %d %d %d %d %d\n", xstart, xend, pw, xstart, halfwidth, get_max_bunch(xstart, xend, halfwidth));
#endif

	/*
	 * If get a new crest, then reset xhalfheight_down!
	 */
fprintf(stderr, "@@@ Reset xhalfheight_down to 0 for peak %d\n", Npeaks);
...
static	void peak_start()
{
	...
	fprintf(stderr, "--- segment %d, xstart: %d\n", Iseg, xstart);
	xstart = get_ok_baseline_point(xstart);
	fprintf(stderr, "=== xstart: %d\n", xstart);
...
#ifdef DEBUG_START
		fprintf(stderr, "peak start: %f moved back to %f\n", TIME_FROM_BUNCH(xstart), TIME_FROM_BUNCH(xstart));
#endif
...
#ifdef DEBUG_START
/*	fprintf(stderr, "peak start: %f\n", TIME_FROM_BUNCH(xstart)); */
	fprintf(stderr, "peak start: %d\n", xstart);
#endif
	...
	fprintf(stderr, "Npeaks, Iseg = %d %d\n", Npeaks, Iseg);
...
fprintf(stderr, "set halfheight down to %d\n", i);
...
/*		fprintf(stderr, "Tailing parent resolved end test at %f\n", TIME_FROM_BUNCH(Segment[Iseg].start_bunch)); */
		fprintf(stderr, "Tailing parent resolved end test at %f\n", Segment[Iseg].start_bunch);
	}
#endif

fprintf(stderr, "In resolved_end_test, halfheight_down is %d\n", Peaks[ipeak].xhalfheight_down);

	flat_test = PEAKWIDTH2(ipeak);
fprintf(stderr, "@@@@@@@@@ flat_test is %d\n", flat_test);
...
fprintf(stderr, "@@@@@@@@@ flat_test is %d\n", flat_test);
fprintf(stderr, "halfheight_down %d xcrest %d\n", Peaks[ipeak].xhalfheight_down, Peaks[ipeak].xcrest);
...
fprintf (stderr, "tangent_test %d, neg_length %d\n", tangent_test, neg_length);
...
fprintf(stderr, "tailing parent peak = %d\n", Tailing_parent_peak);
