...
;
;   This procedure is intended to replace CCG_FLASK going forward.  It uses ccg_flask2.py which is under active dev.  It has not been fully tested so many of the options
;   may not work.  jwm - 7/18
;
;   WARNING:  Use caution when calling this procedure.  It may not be 100% call compatible with previous version.
;
;   For example...
;
...
;
; CALLING SEQUENCE:
;   CCG_FLASK, site='brw',sp='co2',arr
;   CCG_FLASK, site='brw',sp='co2,co,ch4',date=[2000,2003],/average,arr
;   CCG_FLASK, site='brw',sp='co2,co,ch4',adate=[2003],arr
;   CCG_FLASK, site='car',alt=[2500,3500],sp='co2c13',project='ccg_aircraft',arr
;   CCG_FLASK, site='alt,brw',date=[2004,2004],sp='co2,co2c13',arr
;   CCG_FLASK, date=[2003,2003],lat=[10,40],lon=[-90,-50],sp='co2',arr
;   CCG_FLASK, site='stm',sp=['co2'],/nb,arr
;   CCG_FLASK, site='car',sp='co',project='ccg_aircraft',date=20040202,pid='205',arr
;   CCG_FLASK, sp='co2,ch4,co',evn=3246,arr
;   CCG_FLASK, sp='co2',date=[2000,2003],status='ongoing,terminated',arr
;
; INPUTS:
...
;                site code, collection date & time, method, parameter, and
...
;
;                data[].<program>.<sp>   -> data value (mixing ratio, per mil)
;                data[].<program>.<sp>flag   -> 3-character selection flag
;
;                where <program> is the program that made the analysis
;                (e.g., ccgg, hats, arl, etc... ) and <sp> is the
;                species formula (e.g., data.co2, data.co2flag).
;
...
;   Modified, JWM, 7/18 (forked to switch to ccg_flask2.py).
;
   ...
   PRO   BUILD_SITE2, data, comment=comment, $
         noprogram=noprogram, tags, a
   ...
   'id','','type','','meth','',$
   'lat',0.0,'lon',0.0,'alt',0.0,'elev',0.0,'intake_ht',0.0,$
   'date',0.0D,'parameter','')
   ...
   FOR i=0L,n-1 DO BEGIN
      ...
      a[i].intake_ht = FLOAT(str[24+shift])
      a[i].evn = LONG(str[25+shift])
      a[i].system = str[26+shift]

      ; If the user requests comments and comments actually exists
      ; then set the comments in the structure

      IF ( KEYWORD_SET(comment) AND nstr GT 27+shift ) THEN BEGIN
         a[i].comment = STRJOIN(str[27+shift:*],' ')
      ENDIF
   ...
   PRO   BUILD_EVENT2, data, sp = sp, comment=comment, tags, a
   ...
   FOR i=0L,ndata-1 DO BEGIN
      ...
      IF ( KEYWORD_SET(comment) AND nstr GT 15 ) THEN BEGIN
         a[i].comment = STRJOIN(str[15:*],' ')
      ENDIF
   ENDFOR
   END

   PRO   BUILD_MERGE2, data, sp = sp, noprogram=noprogram, headers=headers, tags, a
   ...
   PRO   BUILD_PAIRAVG2, data, noprogram=noprogram, tags, a
   ...
   z = CREATE_STRUCT(z,'value',0.0,'flag','','lat',0.0,'lon',0.0,'alt',0.0)
   ...
   FOR i=0L,n-1 DO BEGIN
      str = STRSPLIT(data[i],' ',/EXTRACT)
      a[i].str = STRCOMPRESS(data[i]) ;Remove extraneous whitespace used for formatting
      a[i].code = str[0]
      a[i].yr = FIX(str[1])
      a[i].mo = FIX(str[2])
      a[i].dy = FIX(str[3])
   ...
   IF N_PARAMS() EQ 0 THEN CCG_FATALERR,"Return variable required (CCG_FLASK,/HELP)."
   ...
   IF (KEYWORD_SET(average) or KEYWORD_SET(pairaverage) or KEYWORD_SET(noprogram)) THEN BEGIN
        CCG_FATALERR, 'Average, Pairaverage & noprogram options have not been tested yet and are unavailable'
   ENDIF

   noprogram = 0 ;override.
   ...
   ;jwm - 7/18 - pointing to python version now
   perlsite = dbdir + 'ccg_flask2.py'
   ;
   ; Build argument list
   ;
   args = ''
   ...
   args = (N_ELEMENTS(eargs) GT 1) ? ' --event_constraints="'+STRJOIN(eargs[1:*],'~',/SINGLE)+'"' : ''
   ...
   ;NOT programmed.. needed?  IF noprogram EQ 1 THEN args += ' -noprogram'
   ...
   args += (N_ELEMENTS(dargs) GT 1) ? ' --data_constraints="'+STRJOIN(dargs[1:*],'~',/SINGLE)+'"' : ''
   ...
   ENDIF
   ;NEED to analyze to see if call compatible  IF KEYWORD_SET(pairaverage) THEN args += ' -pairaverage'
   ;IF KEYWORD_SET(pairaverage) THEN args+=' --pairavg'
   ...
   IF (KEYWORD_SET(average)) THEN BEGIN
      BUILD_MERGE2, data, sp = sp, noprogram=noprogram, headers=headers, tags, arr
   ENDIF ELSE IF (KEYWORD_SET(pairaverage)) THEN BEGIN
           BUILD_PAIRAVG2, data, noprogram=noprogram, tags, arr
   ENDIF ELSE BEGIN
      CASE nsp OF
         0:   BUILD_EVENT2, data, comment=comment, tags, arr
         ELSE:   BUILD_SITE2, data, comment=comment, noprogram=noprogram, tags, arr
      ENDCASE
   ENDELSE
END
