...
from dialogs import TankEntryDialog,FlaskEntryDialog
...
from utils import get_resource, get_start_time
...
class mkSysInfo(wx.Panel):
    def __init__(self, parent, gas ):
        ...
        self.valid_routes = config.valid_measurement_paths
    ...
    def set_summary_page(self):
        ...
        colnames = self.magiccdb.get_column_names("sample_info")
        ...
        self.co_secondary_nl = wx.Button(panel, -1, "Add CO Secondary Resp Curve")
        self.dilution_a_nl     = wx.Button(panel, -1, "Add CO dilution A Resp Curve")
        self.dilution_b_nl     = wx.Button(panel, -1, "Add CO dilution B Resp Curve")
        self.co_2011_primary_a_nl   = wx.Button(panel, -1, "Add CO 2011 Primary A Resp Curve")
        self.co_2011_primary_b_nl   = wx.Button(panel, -1, "Add CO 2011 Primary B Resp Curve")
        self.co_2015_primary_a_nl   = wx.Button(panel, -1, "Add CO 2015 Primary A Resp Curve")
        self.co_2015_primary_b_nl   = wx.Button(panel, -1, "Add CO 2015 Primary B Resp Curve")
        self.h2_primary_nl   = wx.Button(panel, -1, "Add H2 Primary Resp Curve")
        ...
        sizer2.Add(self.co_secondary_nl, 0, wx.ALL,3)
        sizer2.Add(self.dilution_a_nl, 0, wx.ALL,3)
        sizer2.Add(self.dilution_b_nl, 0, wx.ALL,3)
        sizer2.Add(self.co_2011_primary_a_nl, 0, wx.ALL,3)
        sizer2.Add(self.co_2011_primary_b_nl, 0, wx.ALL,3)
        sizer2.Add(self.co_2015_primary_a_nl, 0, wx.ALL,3)
        sizer2.Add(self.co_2015_primary_b_nl, 0, wx.ALL,3)
        sizer2.Add(self.h2_primary_nl, 0, wx.ALL,3)
        ...
        self.Bind(wx.EVT_BUTTON, self.add_CO_SecondaryNL, self.co_secondary_nl)
        self.Bind(wx.EVT_BUTTON, self.add_Dilution_A_NL, self.dilution_a_nl)
        self.Bind(wx.EVT_BUTTON, self.add_Dilution_B_NL, self.dilution_b_nl)
        self.Bind(wx.EVT_BUTTON, self.add_CO_2011_Primary_A_NL, self.co_2011_primary_a_nl)
        self.Bind(wx.EVT_BUTTON, self.add_CO_2011_Primary_B_NL, self.co_2011_primary_b_nl)
        self.Bind(wx.EVT_BUTTON, self.add_CO_2015_Primary_A_NL, self.co_2015_primary_a_nl)
        self.Bind(wx.EVT_BUTTON, self.add_CO_2015_Primary_B_NL, self.co_2015_primary_b_nl)
        self.Bind(wx.EVT_BUTTON, self.add_H2_PrimaryNL, self.h2_primary_nl)
    ...
    def updateFinishTime(self):
        ...
        query = "select AVG(TIMESTAMPDIFF(SECOND,start_datetime,end_datetime)) from ccgg.flask_analysis"
        query += " where start_datetime > '%s' and start_datetime < '%s';" % (dt1, dt2)
        #print(query)
        try:
            rtn = db.doquery(query=query)
            #print(type(rtn))
            #print(rtn)
            #print(rtn[0])
            for key in rtn[0].keys():
                #print("key: %s     val: %s " % (key, rtn[0][key]))
                MINUTES_PER_SAMPLE = float(rtn[0][key]) / 60.0
        except:
            MINUTES_PER_SAMPLE = 8.0
  
        regulator_flush_time = 3.2
        sys_evac_time = 1.9
        process_data_time = 0.5
        pfp_extra_time = 1.0 # pfp cycle slightly longer due to manifold evac, add one minute per sample

        data = self.magiccdb.get_analysis_info()
        ntodo = 0
        extra_time = 0.0
        ...
        for n, line in enumerate(data):
            #print("%s %s %s %s %s %s %s %s %s %s %s %s %s %s " % (line[0], line[1],line[2],line[3],line[4],line[5],line[6],line[7],line[8],line[9],line[10],line[11],line[12],line[13]), file=sys.stderr)


            if n == 0 and line[12].lower() == "ready":
                       ntodo += 1 # add one for initial R0

            if line[12].lower() == "complete" or line[12].lower() == "error":
                previous_sample_type = line[5]
                previous_info_id = line[1]
                continue

            elif line[12].lower() == "running":
                # option: get time diff (now - cycle_start) and add to the extra_time
                if line[5].lower() != "warmup": 
                    ntodo += 1 # add one for following R0 if not warmup mode
            else:
                ntodo += 1 # add one for sample

                if line[5].lower() != "warmup": ntodo += 1 # add one for following R0 if not warmup mode

                if line[5].lower() == "pfp": 
                    extra_time += pfp_extra_time
                ...
                # and add in 3 minute regulator flush. 
                if line[5].upper() == "CAL" and line[1] != previous_info_id:
                    ...
                    extra_time += process_data_time
                    if previous_sample_type.upper() == "CAL":
                        ntodo += 1
        ...
        if ntodo == 0:
            ...
            #print("MINUTES_PER_SAMPLE: %s" % MINUTES_PER_SAMPLE, file=sys.stderr)
            total_minutes=MINUTES_PER_SAMPLE*ntodo+extra_time
            #print("total_minutes: %s" % total_minutes, file=sys.stderr)
            now = datetime.datetime.now()
            td = datetime.timedelta(minutes=total_minutes)  
            s = now + td
    ...
    def updateListBox(self):
        ...
        for n, line in enumerate(data):
            ...
            if sample_type == "flask" or sample_type == "pfp":
                ...
                rid = result[0]["id"]
                station = result[0]["code"]
                sample_date = result[0]["date"]
                sample_time = result[0]["time"]
                sample_method = result[0]["me"]
...
#        db.close()
    ...
    #           if eventnum is not None:
                ...
                rowid = self.magiccdb.insert_entry(sample_info, analysis_info)
    ...
    def editFlaskEntry(self, rowid):
        ...
        if val == wx.ID_OK:
            for (manifold, port, flaskid, eventnum) in dlg.data:
                ...
                sample_info = (manifold, port, flaskid, "flask", num_aliquots, "", "")
                analysis_info = (sample_number, flaskid, eventnum)
                self.magiccdb.update_entry(rowid, sample_info, analysis_info)
                self.updateListBox()
    ...
    def test_nl(self):
        ...
        #sizer2.Add(self.h2_primary_nl, 0, wx.ALL,3)
        if nl_test:
            self.co_secondary_nl.Enable(False)
            self.dilution_a_nl.Enable(False)
            self.dilution_b_nl.Enable(False)
            self.co_2011_primary_a_nl.Enable(False)
            self.co_2011_primary_b_nl.Enable(False)
            self.co_2015_primary_a_nl.Enable(False)
            self.co_2015_primary_b_nl.Enable(False)
            self.h2_primary_nl.Enable(False)
            ...
            self.co_secondary_nl.Enable(True)
            self.dilution_a_nl.Enable(True)
            self.dilution_b_nl.Enable(True)
            self.co_2011_primary_a_nl.Enable(True)
            self.co_2011_primary_b_nl.Enable(True)
            self.co_2015_primary_a_nl.Enable(True)
            self.co_2015_primary_b_nl.Enable(True)
            self.h2_primary_nl.Enable(True)
    ...
    #--------------------------------------------------------------
    def add_CO_SecondaryNL(self, evt):
    ...
    #--------------------------------------------------------------
    def add_CO_2011_Primary_A_NL(self, evt):
        """ Add entries for 2011 primary A set response curve"""
        stdset = "co_primary_2011_a"
        self.addNLEntry(stdset)
    ...
    #--------------------------------------------------------------
    def add_CO_2011_Primary_B_NL(self, evt):
        """ Add entries for 2011 primary B set response curve"""
        stdset = "co_primary_2011_b"
        self.addNLEntry(stdset)
    ...
    #--------------------------------------------------------------
    def add_CO_2015_Primary_A_NL(self, evt):
        """ Add entries for 2015 primary A set response curve"""
        stdset = "co_primary_2015_a"
        self.addNLEntry(stdset)
    ...
    #--------------------------------------------------------------
    def add_CO_2015_Primary_B_NL(self, evt):
        """ Add entries for 2015 primary B set response curve"""
        stdset = "co_primary_2015_b"
        self.addNLEntry(stdset)
    ...
    #--------------------------------------------------------------
    def add_Dilution_A_NL(self, evt):
        """ Add entries for normal std set response curve"""
        stdset = "co_dilution_a"
        self.addNLEntry(stdset)
    ...
    #--------------------------------------------------------------
    def add_Dilution_B_NL(self, evt):
        """ Add entries for normal std set response curve"""
        stdset = "co_dilution_b"
        self.addNLEntry(stdset)
    ...
    def addNLEntry(self, stdset):
       ...
       # update database with analysis information for the response curve cal
        ...
        self.magiccdb.insert_nl_entries(data, num_cycles)
    ...
    def deleteEntry(self, evt):
        ...
        if dlg.ShowModal() == wx.ID_YES:
            self.magiccdb.delete_entry(rowid)
            self.updateListBox()
...
#        print("in get_routes")
#        print(result)
#        print("in get_routes   type(result): %s" % type(result))
#        print("in get_routes   result:", result)
#        if result:
#            for line in result:
#                num = line["num"]
#                routes.append(num)
