...
pages = { 0: 'Aerodyne', 1: 'Picarro', 2: 'SF6', 3: 'H2', 4: 'QC', 5: 'trap_dry_qc'}
...
class mkSignalPage(wx.Panel):
    def __init__(self, parent, gas ):
        ...
        for pagenum, pagelabel in pages.items():
            ...
            # Aerodyne
            if pagenum == 0:
                choices = ["N2O", "CO", "Cell_Press", "Cell_Press_SD", "Cell_Temp","Flow", "Smpl_Press", "N2O_ratio",
                    "CO_ratio", "N2O_diff", "CO_diff", "Analysis_time_delta"]
                self.rb[pagenum] = wx.Choice(panel, -1, choices=choices)

            # Picarro
            elif pagenum == 1:
                choices = ["CO2", "CH4", "H2O", "Cell_Press", "Cell_Press_SD", "Cell_Temp", "Flow", "Smpl_Press", "CO2_ratio",
                    "CH4_ratio", "Analysis_time_delta"]
                self.rb[pagenum] = wx.Choice(panel, -1, choices=choices)

            # SF6
            elif pagenum == 2:
                choices = ["Peak Height", "Peak Area", "Retention time", "Sample Loop Pressure",
                    "Sample_loop_relax","press_correct_area", "Analysis_time_delta"]
                self.rb[pagenum] = wx.Choice(panel, -1, choices=choices)

            # H2 
            elif pagenum == 3:
                choices = ["Peak Height", "Peak Area", "Retention time", "Sample Loop Pressure",
                    "Sample_loop_relax", "press_correct_area","press_correction","press_correction_percent",
                    "area_ratio", "corr_area_ratio", "Analysis_time_delta"]
                self.rb[pagenum] = wx.Choice(panel, -1, choices=choices)

            # QC
            elif pagenum == 4:
                choices = ["cycle_time","init_press", "final_press", "init_evac_time", "init_evac_P", "evac_time",
                    "evac_P", "room_T", "room_P", "idle_P", "chiller_T", "scroll_pump_P","gas_usage",
                    "port_evac_1Torr", "port_evac_100mTorr", "port_evac_P", "port_evac_time"]
                self.rb[pagenum] = wx.Choice(panel, -1, choices=choices)

            elif pagenum == 5:
                choices = [ "flow", "room_T", "chiller_T", "humidity"]
                self.rb[pagenum] = wx.Choice(panel, -1, choices=choices)
    ...
    def doPlotSignal(self, gas, graph, type):
        ...
        for linenum, line in enumerate(raw):
            ...
            if gas.lower() == "aerodyne":
                #[magicc@magicc-3 panel]$ more ../data.aerodyne
                #REF          J0 2024 12 16 09 02 41     
                #333.6358       0.0536 30    
                #279.4220       0.1855 30     
                #294.651        0.007 30    
                #50.051        0.001 30    
                #660.22     67.07     24.40 317
                #[magicc@magicc-3 panel]$ 
                ...
                elif type == "Smpl_Press": v = float(list[20])
                elif type == "Flow":       v = float(list[21])
                elif type == "Analysis_time_delta": v = int(list[23])
                elif type == "N2O_ratio": 
            ...
            if gas.lower() == "h2":
                ...
                #"H2_press_correct_area","press_correction","press_correction_percent"]
                #"area_ratio", "corr_area_ratio"]

            # OUTPUT LINE
            #ref r0 YR MO DY HR MN SC MANIFOLD PORT INIT_PRESS FINAL_PRESS EVAC_TIME EVAC_PRESS ROOM_TEMP ROOM_PRESS IDLE_PRESS CHILLER_TEMP SCROLL_PUMP_P
            # "init_press", "final_press",
            #"evac_time","evac_P",
            #"room_T", "room_P",
            #"idle_P",
            #"chiller_T"])
...
#            xp = decimalDate(yr, mn, dy, hr, mi)
            if gas_type == "SMP":
                ...
                if type != "CO2_ratio" and type != "N2O_ratio" and type != "CO_ratio" and type != "CH4_ratio" and type != "N2O_diff" and type != "CO_diff" and type != "area_ratio" and type != "corr_area_ratio" and type != "port_evac_1Torr" and type != "port_evac_100mTorr" and type != "port_evac_P" and type != "port_evac_time":

                    if gas_name.lower() == "j0":
                        x3.append(t)
                        y3.append(v)
                    else:
                        x2.append(t)
