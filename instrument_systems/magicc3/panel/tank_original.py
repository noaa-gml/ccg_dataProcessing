...
#        self.tankfile = "/home/co2cal/panel.tst" + "/sys.tank_list"
#        self.tankfile_done = "/home/co2cal/panel.tst" + "/sys.tank_list_done"
    ...
    def mkListBox(self):
        ...
        n = 0
        # 'Completed' MUST be last column
        for name in ["Port #", "Serial_Number", "Pressure", "Regulator", "# Aliquots", "Req. #", "Completed"]:
        ...
        for line in lines:
            data = line.split()
            index = listbox.InsertStringItem(nr, data[0])
            listbox.SetItemFont(index, font)
            for i in range(1, len(data)):
                listbox.SetStringItem(index, i, data[i])
...
#    listbox.SetColumnWidth(6, -2)
    ...
    def check_done(self, listbox):
        ...
        numcols = listbox.GetColumnCount()
        for row in range(0, nitems):

            item = listbox.GetItem(row, 1)
            sernum = item.GetText()
            listbox.SetStringItem(row, numcols-1, "")
            for line in lines:
                (portnum, sn, press, reg, naliq, req_n ) = line.split()
                if sernum == sn:
                    listbox.SetStringItem(row, numcols-1, "Completed")
                    break
...
#        dlg.CenterOnScreen()
    ...
    def add_entry_to_list(self, data, index=None):
        ...
        if index is None:
            index = self.listbox.InsertStringItem(n, portnum)
        ...
        for i in range(1,len(data)):
            self.listbox.SetStringItem(index, i, data[i])
...
class TankEntryDialog(wx.Dialog):
    ...
    #---
        ...
        label = wx.StaticText(self, -1, "Request Number: ")
        #box1.Add(label, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.RIGHT, 5)
        box1.Add(label, 0, wx.RIGHT, 5)
        tc = wx.TextCtrl(self, -1, reqnum, validator=Validator(V_INT) )
        self.tclist.append(tc)
        box1.Add(tc, 1, wx.ALIGN_RIGHT|wx.ALL|wx.GROW, 0)
        tc.Enable(False)
...
class SortableListCtrl(wx.ListCtrl, listmix.ColumnSorterMixin):
    ...
    def getData(self, searchstr=None):
        ...
        db, c = ccg_dbutils.dbConnect("refgas_orders")
