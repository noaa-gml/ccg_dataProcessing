...
## read conf file
#devices, virtual_devices, resources = get_resources(config.CONFFILE)
...
try:
    manifold, portnum = utils.resources[config.ref_name.lower()].split()
except:
...
# dictionary returned for samples from the magiccdb using get_next()
try:
    manifold, portnum = utils.resources[config.junk_name.lower()].split()
except:
...
def pre_analysis_check(syslist, sysgases):
   ...
   #test for successful system evacuation
    ...
    try:
           f = open(utils.resources["initial_sys_evac_qc_datafile"], "w")
           f.write("%s   %s\n" % (evac_time, evac_press))
           f.close()
    except:
        ShowStatus("Could not open file %s for writing, exiting ..." % utils.resources["initial_sys_evac_qc_datafile"])
        sys.exit()
...
def continuous_shot(syslist, sysgases):
    ...
    try:
       ...
       ShowStatus("INITIAL  Could not open file %s for writing, exiting ..." % utils.resources["initial_sys_evac_qc_datafile"])
       sys.exit()
