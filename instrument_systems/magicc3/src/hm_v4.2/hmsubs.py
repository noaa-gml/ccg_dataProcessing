...
#        print(virtual_devices)
    ...
    def lock_interface(self, device, option):
        ...
        if option.lower() == "lock" and device.locked is False:
            device.lock()
            device.locked = True
            self.logger.info("LockInterface %s %s", device.name, option)
    ...
    def print_reply(self, device, option):
        ...
        answer = self.query_device(device, option)
        print(answer)
...
#                answer = lgr_test("N2O")
        ...
        try:
            ...
            self.logger.error("*** Split in MonitorNonPolledData FAILED on %s", answer)
            print("ERROR: *** Split in MonitorNonPolledData FAILED on %s" % answer, file=sys.stderr)
            return None
