...
sub get_as_prompt()
{
   ...
   my $log = 'l1_get_as_prompt.log';
   open(my $fh,'>>',$log) or die "Couldn't open log file $log";
   print $fh "start get_as_prompt at: ".localtime()."\n";
 
   while ( $try < $maxtries && $prompt eq '' )
   {
      print $fh "try before as_send_and_read: ".$try."  ".localtime()."\n";
      @reply = &as_send_and_read($device, '2');
      print $fh "try after as_send_and_read: ".$try."  ".localtime()."\n";
