...
def shutdown():
    ...
    s = "Magicc system stopped at %s" % now.strftime("%c")
    ShowStatus(s)
...
def get_systems():
        ...
        for name in ["gc1","gc2","aerodyne","picarro"]:
...
def rotate_logs(logfile, count):
    ...
    for i in range(count-1, 0, -1):
        ...
        src="%s/%s.%03d" % (config.LOGBACKUPDIR, os.path.basename(logfile), i)
        dest = "%s/%s.%03d" % (config.LOGBACKUPDIR, os.path.basename(logfile), i+1)
    ...
    if os.path.exists(logfile):
        ...
        dest = "%s/%s.001" % (config.LOGBACKUPDIR, os.path.basename(logfile))
        #os.rename(logfile, dest)
        #for log backup on different file system use these
...
try:
    ...
    s = "Magicc is already running. Exiting..."
    sys.exit(s)
...
logging.info("Magicc System startup")
...
if mode != 6:  
    ...
    if "aerodyne" in syslist:
        ...
        try:
            os.remove(utils.get_resource("aerodyne_setup_script_ack"))
        except:
            pass
        ...
        while runscript_accepted == False and try_cnt < 5:
            ...
            if os.path.isfile(utils.get_resource("aerodyne_setup_script_ack")): runscript_accepted = True
