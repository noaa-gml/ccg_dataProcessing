...
class PreferencesDialog(wx.Dialog):
    """ Dialog for changing the settings of the graph """
    ...
    def makeGridPage(self, nb):
        ...
        #----- 
        box1 = wx.FlexGridSizer(0, 2, 2, 2)
        box0.Add(box1, 0, wx.GROW|wx.ALL, 5)
        ...
        for axis in self.graph.axes:
            if axis.isXAxis():
                s = "X%d" % axis.id
                choicelist.append(s)
        self.gridXaxis = wx.Choice(page, -1, choices=choicelist)
        ...
        for axis in self.graph.axes:
            if axis.isYAxis():
                s = "Y%d" % axis.id
                choicelist.append(s)
        self.gridYaxis = wx.Choice(page, -1, choices=choicelist)
        ...
        axis = self.graph.getXAxis(0)
        showgrid = axis.show_grid
        self.show_grid = wx.CheckBox(page, -1, "Show Grid")
        self.show_grid.SetValue(showgrid)
        sizer2.Add(self.show_grid, 0, wx.GROW|wx.ALL, 2)

        box1 = wx.FlexGridSizer(0,2,2,2)
        sizer2.Add(box1, wx.GROW|wx.ALIGN_RIGHT|wx.ALL)
        label = wx.StaticText(page, -1, "Grid Color:")
        box1.Add(label, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL, 2)
        color = axis.grid_pen.GetColor()
        ...
        box1.Add(label, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL, 2)
        width = axis.grid_pen.GetWidth()
        self.grid_linewidth = wx.SpinCtrl(page, -1, str(width), size=(50, -1))
        box1.Add(self.grid_linewidth, 0, wx.ALIGN_LEFT|wx.ALL, 2)

        label = wx.StaticText(page, -1, "Line Type:")
        box1.Add(label, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL, 2)
        self.grid_type = wx.Choice(page, -1, choices=list(LINE_TYPES.keys()))
        value = StyleToName(axis.grid_pen.GetStyle())
        ...
        axis = self.graph.getXAxis(0)
        showsubgrid = axis.show_subgrid
        self.show_subgrid = wx.CheckBox(page, -1, "Show SubGrid")
        self.show_subgrid.SetValue(showsubgrid)
        sizer2.Add(self.show_subgrid, 0, wx.GROW|wx.ALL, 2)

        box1 = wx.FlexGridSizer(0, 2, 2, 2)
        sizer2.Add(box1, wx.GROW|wx.ALIGN_RIGHT|wx.ALL)
        label = wx.StaticText(page, -1, "SubGrid Color:")
        box1.Add(label, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL, 2)
        color = axis.subgrid_pen.GetColour()
        self.subgrid_color = wx.ColourPickerCtrl(page, -1, color)
        box1.Add(self.subgrid_color, 0, wx.ALIGN_LEFT|wx.ALL, 2)

        label = wx.StaticText(page, -1, "Line Width:")
        box1.Add(label, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL, 2)
        width = axis.subgrid_pen.GetWidth()
        self.subgrid_linewidth = wx.SpinCtrl(page, -1, str(width), size=(50,-1))
        box1.Add(self.subgrid_linewidth, 0, wx.ALIGN_LEFT|wx.ALL, 2)

        label = wx.StaticText(page, -1, "Line Type:")
        box1.Add(label, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL, 2)
        self.subgrid_type = wx.Choice(page, -1, choices=list(LINE_TYPES.keys()))
        value = StyleToName(axis.subgrid_pen.GetStyle())
    ...
    def makeCrosshairPage(self, nb):
        ...
        #-----
        box = wx.StaticBox(page, -1, "Crosshairs")
        sizer2 = wx.StaticBoxSizer(box, wx.VERTICAL)
        box0.Add(sizer2, 0, wx.EXPAND|wx.ALIGN_LEFT|wx.ALL, 5)

        box1 = wx.FlexGridSizer(0,2,2,2)
        sizer2.Add(box1, 0, wx.GROW|wx.ALL, 5)
    ...
    #                box0.Add(box1, wx.GROW|wx.ALIGN_RIGHT|wx.ALL)
        ...
        if wx.Platform != "__WXMAC__":
            ...
            sizer2.Add(self.show_chlabel, 0, wx.GROW|wx.ALL, 2)
            ...
            for axis in self.graph.axes:
                if axis.isXAxis():
                    s = "X%d" % axis.id
                    choicelist.append(s)
            self.chXaxis = wx.Choice(page, -1, choices=choicelist)
            xaxis_id = self.graph.popup.xaxis
            s = "X%d" % xaxis_id
            ...
            for axis in self.graph.axes:
                if axis.isYAxis():
                    s = "Y%d" % axis.id
                    choicelist.append(s)
            self.chYaxis = wx.Choice(page, -1, choices=choicelist)
            yaxis_id = self.graph.popup.yaxis
            s = "Y%d" % yaxis_id
    ...
    def makeTitlePage(self, nb):
        ...
        self.title_font.SetSelectedFont(self.graph.title.font.wxFont())
    ...
    def makeLegendPage(self, nb):
        ...
        sizer2.Add(self.show_legend, 0, wx.GROW|wx.ALL, 2)
        ...
        self.legend_font.SetSelectedFont(self.graph.legend.font.wxFont())
        f = self.graph.legend.font.wxFont()
        ...
        box1.Add(label, 0, wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL, 2)
        choicelist = ["Right", "Left", "Top", "Bottom"]
        self.legend_location = wx.Choice(page, -1, choices=choicelist)
        ...
        sizer2.Add(self.show_legend_border, 0, wx.GROW|wx.ALL, 2)
    ...
    def apply(self, event):
        ...
        axis_id = int(val[1])
        xaxis = self.graph.getXAxis(axis_id)

        val = self.gridYaxis.GetStringSelection()
        axis_id = int(val[1])
        yaxis = self.graph.getYAxis(axis_id)
        ...
        pentype = NameToStyle(val)
        xaxis.grid_pen.SetPen(color, width, pentype)
        yaxis.grid_pen.SetPen(color, width, pentype)

        # Get subgrid values
        val = self.show_subgrid.GetValue()
        xaxis.show_subgrid = val
        yaxis.show_subgrid = val

        color = self.subgrid_color.GetColour()
        width = self.subgrid_linewidth.GetValue()
        val = self.subgrid_type.GetStringSelection()
        pentype = NameToStyle(val)
        xaxis.subgrid_pen.SetPen(color, width, pentype)
        yaxis.subgrid_pen.SetPen(color, width, pentype)
        ...
        # Get crosshair popup label values
        if wx.Platform != "__WXMAC__":
            ...
            self.graph.popup.xaxis = int(val[1])
            val = self.chYaxis.GetStringSelection()
            self.graph.popup.yaxis = int(val[1])
    ...
    #        self.graph.title.font = font
        self.graph.title.font.SetFont(font.GetPointSize(), font.GetFamily(), font.GetStyle(), font.GetWeight())
        ...
        self.graph.legend.font.SetFont(font.GetPointSize(), font.GetFamily(), font.GetStyle(), font.GetWeight())
