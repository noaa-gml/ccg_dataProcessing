...
def update_files(inst, sysgases, caltype, stdset="", serialnum="", pressure="", regulator="", manifold="", 
    ...
    if inst.upper() != "QC": 
        ...
        for sp in sysgases[inst]:
            rawfile = utils.get_rawfile_name(sp, inst_id)
            ShowStatus("making raw file for %s %s" % (inst.upper(), sp.upper()))
            utils.make_raw(config.SYSTEM_NAME, inst, caltype, qc=False, gas=sp, rawfile=rawfile, stdset=stdset,
                ...
                manifold="%s" % manifold, portnum=portnum)
            ShowStatus("saving raw file for %s %s" % (inst.upper(), sp.upper()))
            utils.save_files(processtype, inst, sp, rawfile)
    
        # process each rawfile
        for sp in sysgases[inst]:
            rawfile = utils.get_rawfile_name(sp, inst_id)
            ShowStatus("processing raw file for %s %s" % (inst.upper(), sp.upper()))
            year = int(rawfile[0:4])
            process_filename = "/ccg/%s/%s/%s/raw/%d/%s" % (sp.lower(), processtype.lower(), config.SYSTEM_NAME.lower(), year, rawfile)
            utils.process_data(caltype, process_filename, stdset)

            # add manifold/port to reftank.calibrations_qcdata table
            ShowStatus("adding manifold/port info to reftank.calibrations_qcdata table")
            cmd = "python /home/co2cal/src/update_calqc.py -u %s" % (process_filename)
        ...
        for sp in sysgases[inst]:
            ...
            utils.make_raw(config.SYSTEM_NAME, inst, caltype, qc=True, gas=sp, rawfile=qcrawfile, stdset=stdset,
                serialnum=serialnum, pressure=pressure, regulator=regulator, 
                manifold=manifold, portnum=portnum)
...
def evac_system(action, sampledata, limit, msg=""):
    ...
    (vtype, vid, max_positions) = utils.virtual_devices[manifoldname.lower()]['option'].split()
...
##############################################################
def fill_aerodyne(action, gastype, gasname, aerodyne_qc_datafile="aerodyne_qc.dat", background=False):
    """
    ...
    if background = True then do automatic background under evac before filling
    ...
    # Evacuate AR cell
    if background:
        ShowStatus("Aerodyne Evac with N2 purge for background measurement:  %s   %s" % (gastype, gasname))
        action.run("aerodyne_n2_purge_evac.act")
    else:
    ...
    while runscript_accepted == False and try_cnt < 5:
        try_cnt += 1
        # might want to add a limit to how many times it can try, or may get stuck here
        if background:
            ShowStatus("Aerodyne Background / Fill:  %s   %s" % (gastype, gasname))
            action.run("aerodyne_background_fill.act") #3 secs 
        else:
    ...
    fill_time = 20
    if background:
        #aerodyne_abg_duration  aydur10
        j, duration = utils.get_resource("aerodyne_abg_duration").split('r')
        time.sleep(int(fill_time) + 20)
    else:
        time.sleep(fill_time)
    ShowStatus("Finished filling Aerodyne cell")
...
##############################################################
##############################################################
# co2cal-2 version of run_sample
def run_sample(action, syslist, sysgases, referencedata, sampledata=None, runtype="NUL", storegc=False, transducer_offset=0.0, namespace=None):
    """
    ...
    *** change from magicc - both sample and ref passed in and the position value determines which is on the PC vs istotope analyzer
    ...
    --------------------------------------------------------------------------------
    """
    ShowStatus("Start common.py/run_sample ")
    run_smp = True
    run_ref = True
    
    # set label for "sample" gas
    if runtype.lower() == "cal":
        smp_type = "SMP"
    elif runtype.lower() == "nl":
        smp_type = "STD"
    elif runtype.lower() == "warmup":
        smp_type = "warmup"
    else:
        smp_type = "null"
    ...
    # decide if running both smp and ref
    if not sampledata: run_smp = False
    if not referencedata: run_ref = False

    # loop through both position A and B
    for position in ["A","B"]:
        ShowStatus("common.py/run_sample - Begin analysis, position %s.  run_smp: %s   runref: %s " % (position, run_smp, run_ref))
        running_instruments = []
        flow_check = 0
        run_isotopes = True
        run_pc = True
        ...
        ##########################33
        # setup valves based on samples being run, position, and instruments in use
        # -- In position A   REF -> PC    SMP -> isotope analyzer
        if position.upper() == "A": 
            if run_ref and "picarro" in syslist:
                action.run("closerelay.act","@Picarro_idle") #power idle valve to send smp/ref gas to inst
                action.run("openrelay.act", "@Reference_mks_valve") # open relay that closes mks controller valve
                running_instruments.append("picarro")
            else:
                ShowStatus("not running reference gas through picarro, set picarro to idle gas and turn off ref flow at MKS valve")
                action.run("openrelay.act","@Picarro_idle")
                action.run("closerelay.act", "@Reference_mks_valve") 


            if run_smp and ("aerodyne" in syslist or "lgr" in syslist):
                action.run("closerelay.act","@Isotope_idle") #power idle valve to send smp/ref gas to inst
                action.run("openrelay.act", "@Sample_mks_valve")

                if "lgr" in syslist:
                    running_instruments.append("lgr")
                elif "aerodyne" in syslist:
                    running_instruments.append("aerodyne")
            else:
                ShowStatus("not running sample on isotope analyzer, set isotope to idle gas and turn off sample flow at MKS valve ")
                action.run("openrelay.act","@Isotope_idle") #un-power idle valve to send idle gas to inst
                action.run("closerelay.act", "@Sample_mks_valve") #close relay to shut valve in MKS controller

            
        # In position B     SMP -> PC     REF -> isotope analyzer        
        if position.upper() == "B":   
            if run_ref and ("aerodyne" in syslist or "lgr" in syslist):
                action.run("closerelay.act","@Isotope_idle") #power on idle valve to send smp/ref gas to inst
                action.run("openrelay.act", "@Reference_mks_valve") #open relay that closes mks controller valve
                ...
                ShowStatus("not running ref through isotope analyzer, set isotope analyzer to idle gas and turn off ref flow at MKS valve")
                action.run("openrelay.act","@Isotope_idle") #un-power idle valve to send idle gas to inst
                action.run("closerelay.act", "@Reference_mks_valve") #close relay to shut valve in MKS controller
            
            if run_smp and "picarro" in syslist:
                action.run("openrelay.act", "@Sample_mks_valve")
                action.run("closerelay.act","@Picarro_idle")
                running_instruments.append("picarro")
            else:
                ShowStatus("not running sample through picarro, set picarro to idle gas and turn off smp flow at MKS valve")
                action.run("openrelay.act","@Picarro_idle")
                action.run("closerelay.act", "@Sample_mks_valve")
        ##########################33

        # Switch System Select
        action.run("switch_two_position_valve.act", "SysSelect", position)

        ShowStatus("common.py/run_sample - running instruments: %s" % running_instruments)  
        if not running_instruments:
            return 0
            
        #remove temp data files
        if os.path.exists(utils.get_resource("monitor_device_result_file", "hp34970")): 
            os.remove(utils.get_resource("monitor_device_result_file", "hp34970"))
        for inst in syslist:
            
            if os.path.exists(utils.get_resource("monitor_device_save_file", inst)):
                os.remove(utils.get_resource("monitor_device_save_file", inst))

            if os.path.exists(utils.get_resource("monitor_device_result_file", inst)):
                os.remove(utils.get_resource("monitor_device_result_file", inst))

            if os.path.exists(utils.get_resource("qc_file", inst)):
                os.remove(utils.get_resource("qc_file", inst))
        ...
        ## Turn Manifold Select to sample port - this should happen outside of run_sample
        #if run_smp:
        #    #port = "@%s_port" % manifoldname
        ...
        #    action.run("turn_multiposition.act", "Manfold%s" % sampledata["manifold"],sampledata["port_num"])

        # flush systems and start QC data collection by hp34970
        ShowStatus("begin system flush")
        action.run("flush.act")

        # measure
        #for inst in syslist:
        for inst in running_instruments:
            ShowStatus("measure %s" % inst)
            if inst == "picarro":
                # Start Picarro measurments
                proc1 = action.runbg("measure_picarro.act", utils.get_resource("monitor_device_result_file", inst), 
                        utils.get_resource("monitor_device_save_file", inst))
                proc1_on = True

            #Can only run one of the isotope instruments at a time
            if inst == "aerodyne":
                # Start Isotope measurments
                proc2 = action.runbg("measure_aerodyne.act", utils.get_resource("monitor_device_result_file", inst), 
                        utils_get_resource("monitor_device_save_file", inst))
                proc2_on = True

            elif inst == "lgr":
                # Start Isotope measurments
                proc2 = action.runbg("measure_lgr.act", utils.get_resource("monitor_device_result_file", inst), 
                            utils.get_resource("monitor_device_save_file", inst))
                proc2_on = True

        ShowStatus("wait for processes to finish")
        if proc1_on:  proc1.wait()
        if proc2_on:  proc2.wait()

        # end cycle and collect qc data from hp34970
        ShowStatus("end measurement cycle")
        action.run("end_cycle.act", utils.get_resource("monitor_device_result_file", "hp34970"))   

        # set cycle end time
        utils.set_end_time(cycle=True)

        # Convert and Save data
        ShowStatus("convert and save data")
        ...
        # set sample type, id's - include press, flow for later use
        if run_smp:
            smp_data = (smp_type, sampledata["sample_id"], smp_press, smp_flow)
        else:
            smp_data = ("na", "na", -99.9, -99.9)

        if run_ref:
            ref_data = ("REF", referencedata["sample_id"], ref_press, ref_flow)
        else:
            ref_data = ("na", "na", -99.9, -99.9)
        ...
        for inst in running_instruments:
            ...
            if inst == "picarro":
                if position.upper() == "A":
                    (gas_type, gas_name, press, flow) = ref_data
                    if ref_flag != 0:
                        flow_check = 1
                else:
                    (gas_type, gas_name, press, flow) = smp_data
                    if smp_flag != 0:
                        flow_check = 1
            else:
                if position.upper() == "A":
            ...
            #write press and flow with other qc info to qc.dat for use by cvt.dat
            f1 = open(utils.get_resource("qc_file", inst), "w")
            print("-99.9  %s  %s  %s %s 0  0" % (press, flow, rT, cT ), file=f1)
            f1.close()

            #data = utils.cvt_data( inst, temp_datafile, temp_qcfile)
            data = utils.cvt_data( inst, utils.get_resource("monitor_device_result_file", inst) , utils.get_resource("qc_file", inst))
            f = open(utils.get_resource("datafile", inst), "a")
            print("%3s %4s %s" % (gas_type, gas_name, data), file = f)
            f.close()

            # save high frequency data
            f = open(utils.get_resource("high_freq_datafile", inst), "a")
            f2 = open(utils.get_resource("monitor_device_save_file", inst), "r")
            datalines_highfreq = f2.readlines()
            for line in datalines_highfreq:
                print("%3s %4s %s" % (gas_type, gas_name, line.rstrip('\n')), file=f)
            f2.close()
            f.close()
    
    # update sample list database if measuring samples or standards  
    ShowStatus("update run sequence database")
    if sampledata:
        magiccdb.mark_complete(sampledata["analysis_id"])
    elif runtype.lower() == "warmup":
        magiccdb.mark_complete(referencedata["analysis_id"])

    ShowStatus("return from common.py/run_sample")
...
#    aerodyne_qc_datafile =  utils.get_resource("aerodyne_qc_datafile")  #filename follows template "inst"_qc.dat
#    picarro_qc_datafile =  utils.get_resource("picarro_qc_datafile")   #filename follows template "inst"_qc.dat
...
#    utils.set_start_time(cycle=True) 
#
#    # remove aerodyne.dat file so that if we don't measure aerodyne we get 9999's for values rather than last run values
#    if os.path.exists(utils.get_resource("picarro_datafile")): os.remove(utils.get_resource("picarro_datafile"))
#    if os.path.exists(utils.get_resource("picarro_high_freq_datafile")): os.remove(utils.get_resource("picarro_high_freq_datafile"))
#    if os.path.exists(utils.get_resource("picarro_qc_datafile")): os.remove(utils.get_resource("picarro_qc_datafile"))
#    if os.path.exists(utils.get_resource("aerodyne_datafile")): os.remove(utils.get_resource("aerodyne_datafile"))
#    if os.path.exists(utils.get_resource("aerodyne_high_freq_datafile")): os.remove(utils.get_resource("aerodyne_high_freq_datafile"))
#    if os.path.exists(utils.get_resource("aerodyne_qc_datafile")): os.remove(utils.get_resource("aerodyne_qc_datafile"))
...
#        aerodyne_check = fill_aerodyne(action, gastype, sampledata["sample_id"], background=int(run_bkg))
#    else:
#        time.sleep(35)
#
...
#    action.run("test_sample_loop_pressure.act" )
#    (j, sample_loop_press, room_press, j, j) = action.output.split()
#    delta = (float(sample_loop_press) - float(transducer_offset)) - float(room_press)
#    ShowStatus("%s Sample loop pressure (Before venting): %7.2f      Room pressure: %7.2f   Transducer offset: %7.2f   delta: %7.2f" % (
#            status_tag, float(sample_loop_press), float(room_press), float(transducer_offset), delta ))
#    
#    if float(sample_loop_press) - float(transducer_offset) <= float(room_press):
...
#    if relax_gc:
#        transducer_offset = float(sample_loop_press) - float(room_press) 
#
#    ShowStatus("%s Sample loop pressure (After venting): %7.2f      Room pressure: %7.2f   New Transducer offset: %7.2f   " % (
#            status_tag, float(sample_loop_press), float(room_press), float(transducer_offset)))
#
...
#    if "aerodyne" in syslist and aerodyne_check == True:
#        aerodyne_check = fill_aerodyne(action, gastype, sampledata["sample_id"], utils.get_resource("aerodyne_qc_datafile"))
#    else:
...
#            #proc3 = action.runbg("aerodyne_stop_flow_measure.act", "aerodyne.dat")
#            proc3 = action.runbg("aerodyne_stop_flow_measure.act", utils.get_resource("aerodyne_datafile"), 
#                utils.get_resource("aerodyne_high_freq_datafile"))
#            proc3_on = True
#        else:
#            ShowStatus("aerodyne_check failed, not communicating with the Aerodyne, skip all remaining samples")
#            # mark everything as error so files will be processed and code will exit cleanly
#            # get all entries from db, check each one, if ready then mark as error
#            rows = magiccdb.get_analysis_info()
#            for line in rows:
#                    if line[12].upper() != "COMPLETE":
#                            magiccdb.mark_error(line[0])
#        
...
#        # measure picarro
#        ShowStatus("measure_picarro.act:  %s" % (status_tag))
#        #action.run("measure_picarro.act", "picarro.dat", utils.get_resource("picarro_qc_datafile")) 
#        action.run("measure_picarro.act", utils.get_resource("picarro_datafile"), utils.get_resource("picarro_qc_datafile"), 
#            utils.get_resource("picarro_high_freq_datafile")) 
#
...
#    if proc1_on:  proc1.wait()
#    if proc2_on:  proc2.wait()
#    #if proc3_on:  proc3.wait()  #don't wait for AR, when crashes due to comm failure we need to move on
#
...
#    for inst in syslist:
#        inst_id, sernum = utils.get_instrument_id(inst)
#        ShowStatus("process data into data.* files:  %s" % (status_tag))
...
#
#            #data = utils.cvt_data( inst, tmp_datafile, tmp_qc_datafile)
...
#            #f2 = open(tmp_high_freq_datafile, "r")
#            f2 = open(utils.get_resource("%s_high_freq_datafile" % inst.lower()), "r")
#            high_freq_data = f2.readlines()
#            f2.close()
#            f = open(high_freq_datafile, "a")
#            for line in high_freq_data:
#                #print >> f, "%s %s" % (prefix, line.rstrip())
...
#   # test time pfp took to hit 1 Torr to check for open flasks
    ...
    try:
           #f = open(utils.get_resource("tmp_pfp_qc_datafile"), "w")
...
def query_pfp_splitter(action, manifold):
    ...
    cmd = "STATUS" 
    action.run("query_pfp_comm_splitter.act", device_name, cmd) 
...
def getPFPID(action, manifold, port, pfpid):
    ...
    try:
        returned_id = subprocess.check_output("/home/magicc/bin/get_as_serial.pl %s" % devfile, shell=True)
