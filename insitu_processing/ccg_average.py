    ...
    def doHourlyAverage(self, option=None, selection=True):
        ...
        if selection is True:
            for intake_height in data:
                data[intake_height] = self._selectData(data[intake_height])
...
#                    mean = d.mean()  # should this be weighted avg using d, w?
...
#                    if inst_num is None: inst_num = -1
                    inst_num = rows[0]['inst_num']
...
#            default_data[intake_height][date] = tmp_data
...
#        result = self.db.doquery(sql, (datestr, self.sitenum, self.paramnum, system, instnum))
    ...
    def doDailyAverage(self, option):
        ...
        # create a dict with default values for every hour of the year
...
#        sys.exit()
        ...
        if result:
            ...
            for row in result:
                ...
                t = (date, intake_height, val, sdev, n, flag)
#                t = (intake_height, val, sdev, n, flag)
#                data[date] = t
                data.append(t)
        ...
        if option == CHECK:
            ...
            for t in sorted(data):
                (dt, intake_ht, mr, mrsd, nv, flag) = t
                print(self.RESULT_FORMAT_DAY % (self.ucstacode, dt.year, dt.month, dt.day, mr, mrsd, nv, flag, intake_ht))
#            for dt in sorted(data):
#                (intake_ht, mr, mrsd, nv, flag) = data[dt]
#                print(self.RESULT_FORMAT_DAY % (self.ucstacode, dt.year, dt.month, dt.day, mr, mrsd, nv, flag, intake_ht))
...
#        for date in sorted(data):
            (date, intake_ht, mr, mrsd, nv, flag) = t
#            (intake_ht, mr, mrsd, nv, flag) = data[date]
    ...
    def _update_day_db(self, data):
        ...
        for t in sorted(data):
#        for date in sorted(data):
            (date, intake_ht, val, sdev, n, flag) = t
#            (intake_ht, val, sdev, n, flag) = data[date]
    ...
    def doMonthlyAverage(self, option):
        ...
        if option == CHECK:
            ...
            for t in sorted(data):
                (date, intake_height, val, sdev, n, flag) = t
                yr = date.year
                mon = date.month
    ...
    def _check_month_db(self, data):
        ...
        for t in sorted(data):
            (dt, intake_ht, mr, mrsd, nv, flag) = t
            date = "%d-%d-%d" % (dt.year, dt.month, 15)
            ...
            if result:
                ...
                diff = mr - mr2
                if abs(diff) > 0.011:
                    line = self.RESULT_FORMAT_MONTH % (self.stacode, dt.year, dt.month, mr, mrsd, nv, flag, intake_ht)
                    print("%s mixing ratio mismatch (%8.2f, %6.2f)." % (line, mr2, diff))
                else:
                    if self.verbose:
    ...
    def _doCH4MonthlyAverage(self, df):
        ...
        intake_heights = numpy.unique(df['intake_ht'])

        for intake_height in intake_heights:
            w = numpy.where(df['intake_ht'] == intake_height)

            x = [ccg_dates.decimalDateFromDatetime(t) for t in df['date'][w]]
            y = df['value'][w]
            ...
            for month in range(1, 13):
                ...
                date = datetime.datetime(self.year, month, 1)
                intake_height = self.intakes.get_intake("Line1", date)

                if avg >-900:
                    t = (date, intake_height, avg, sdev, n, flag)
                    data.append(t)
