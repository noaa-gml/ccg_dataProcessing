...
class ndir2:
    ...
    def __init__(self, stacode, gas, raw, system, refgas, smptype="SMP", debug=False):
        ...
        # get dates when tanks or instruments change
...
#            print(ysd)
    ...
    def _check_cal(self, start_cal, df, std_labels):
        ...
        # make sure we have all r0,s1,s2,s3,s4,r0 measurements with good flags

        # check for towers where the reference is one of the standards
        if self.reference not in self.std_labels:   # non-tower
            start_cal = start_cal - 1               # where first R0 is
            firstindex = df.index[0]                # index number of first row in dataframe
            if firstindex > start_cal:              # no R0 before S1
                return False
            labels = [self.reference] + std_labels + [self.reference]
        else:
            labels = std_labels
