...
class picarro(hmsubs.hmsub):
    ...
    def _setValues(self, v, values):
        ...
        if self.config:
            water_corrected = int(self.config['DEFAULT']['water_corrected'])
        else:
            water_corrected = 0
        species_column = 6
...
#        result = "%s" % dt
    ...
    #########################################################################
    def get_picarro_buffer(self, device, option):
        """
        ...
        try:
            ...
            if len(vals) > 3:
                ...
                if a not in ("0", "-1"):
                    ...
                    try:
                        ...
                        self._error_exit("could not open file %s" % filename)
            ...
            self._error_exit("GetPicarroBuffer error for %s. Incorrect option string '%s'." % (device.name, option))
        ...
        if return_all:
            ...
            for values in v:
                avg, std, num = self._meanstdv(values)
                result += " %f %f %d" % (avg, std, num)
