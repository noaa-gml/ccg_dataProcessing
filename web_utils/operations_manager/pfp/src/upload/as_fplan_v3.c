...
int main(int argc, char *argv[])
{
  ...
  char sum[maxline];
  char altitude[maxline];
  char latitude[maxline];
  char longitude[maxline];
  char datestr[maxline];
  char timestr[maxline];
  char yearstr[maxline];
  char monthstr[maxline];
  char daystr[maxline];
  char hourstr[maxline];
  char minstr[maxline];
  char mon[3], min[3];
  char *stoken[100];
	 ...
	 while(n>=0)                             /*Read each line of the input file, parse it, and send to PFP*/
	   {
		...
		printf("longitude=%s, date is %s, time is %s\n", stoken[3], stoken[4], stoken[5]); */
		/* sum=strcat(stoken[0],"");
		...
		strcpy(sum, stoken[0]);
		strcpy(altitude, stoken[1]);
		strcpy(latitude, stoken[2]);
		strcpy(longitude, stoken[3]);
		strcpy(datestr, stoken[4]);
		strcpy(timestr, stoken[5]);
		i=0;
		j=0;
		b=0;
