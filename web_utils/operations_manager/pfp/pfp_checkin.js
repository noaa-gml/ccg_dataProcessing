...
//var return_notes = new Array();
//var routing_notes = new Array();
var site_notes = new Array();
...
function FlaskSelection(id)
{
   ...
   {
      if (f.selectedlist.length == 1) { alert('Check in one PFP at a time.');return; }
   ...
   { oflask_notes.innerHTML = '** PFP **<BR>['+id+'] '+flask_notes[availablelistindex]+'<P>'; }
...
function SiteListCB()
{
   ...
   f.site_notes.value = site_notes[i];
...
function OkayCB()
{
   ...
   {
      f.id.value = f.selectedflasks.value;
      f.action = omurl+'pfp/pfp_download.php';
      f.submit();
   }
   else { return; }
}
