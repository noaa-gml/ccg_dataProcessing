...
function RecallCB()
{
   ...
   if (f.ev_temp.value == '') { f.ev_temp.value = f.last_temp.value; }
   if (f.ev_press.value == '') { f.ev_press.value = f.last_press.value; }
   if (f.ev_rh.value == '') { f.ev_rh.value = f.last_rh.value; }
   ...
   if (f.ev_alt_source.value == '') { setSelectValue(f.ev_alt_source,f.last_alt_source.value); }
   setSelectValue(f.ev_elev_source,f.last_elev_source.value);
...
function BuildEventString()
{
   ...
   var lat, lon;
   // temp, press, rh
   if ( f.ev_temp.value == defaults1.temp ) { f.ev_temp.value = ''; }
   if ( f.ev_press.value == defaults1.press ) { f.ev_press.value = ''; }
   if ( f.ev_rh.value == defaults1.rh ) { f.ev_rh.value = ''; }
   ...
   var commentstr = '';
   var alt_source = getSelectValue(f.ev_alt_source);
   if ( alt_source != '' )
   {
      if ( f.ev_comment.value != '' )
      {
         commentstr = 'alt:'+alt_source+'~+~'+f.ev_comment.value;
      }
      else
      {
         commentstr = 'alt:'+alt_source;
      }
   }
   else
   {
      commentstr = f.ev_comment.value;
   }
   var elev_source = getSelectValue(f.ev_elev_source);
   commentstr = 'elev:'+elev_source+'~+~'+commentstr;
...
function SetDefaultsCB()
{
   ...
   if (f.ev_temp.value == '') { f.ev_temp.value = defaults1.temp; }
   if (f.ev_press.value == '') { f.ev_press.value = defaults1.press; }
   if (f.ev_rh.value == '') { f.ev_rh.value = defaults1.rh; }
   ...
   if (f.ev_alt_source.value == '') { setSelectValue(f.ev_alt_source.value,''); }
...
function Help_AltSource()
{

   str = ''
   str = str + 'plan\n';
   str = str + '   - PFP sample plan alt\n';
   str = str + 'plan_edit\n';
   str = str + '   - Modification of the PFP sample plan alt\n';
   str = str + 'gps\n';
   str = str + '   - GPS alt from PFP\n';
   str = str + 'gps_edit\n';
   str = str + '   - Modification of the GPS alt from the PFP\n';
   str = str + 'gps_nav\n';
   str = str + '   - GPS alt from navigation device. Not from PFP.\n';
   str = str + 'gps_nav_edit\n';
   str = str + '   - Modification of the GPS alt from navigation device\n';
   str = str + 'db\n';
   str = str + '   - Alt retrieved from database. No altitude in PFP.\n';
   str = str + 'db_edit\n';
   str = str + '   - Modiication of the alt retrieved from database.';
