...
$perl_readme = "/projects/ftp/readme/data_request/build_readme.pl";
$reqno = int(10**8*rand());
...
sub EmailCollab()
{
   ...
   foreach $item ( @list )
   {
      ...
      if ( index(lc($parameterprogram_str),'hats') != -1) #Klugey way to get ben on hats emails.. see comments in ccg_projectcontact.pl jwm - 1/3/17
		#11/13/2020, switching cc to isaac
      { push(@collab, 'isaac.vimont@noaa.gov'); }
   		#and adding isaac to arl list
      if ( index(lc($parameterprogram_str),'arl') != -1) 
      { push(@collab, 'isaac.vimont@noaa.gov'); } 

    if ( index(lc($parameterprogram_str),'sil') != -1)
      { push(@collab, 'sylvia.michel@Colorado.edu');}
  
    if ( index(lc($parameterprogram_str),'co2c14') != -1){
	push(@collab, 'john.b.miller@noaa.gov');
      }
   }
...
sub SendEmail()
{
   ...
   #For debugging... to route all mail to me, uncomment next 3 lines.
   #$subject="${subject}, ${to} ";
   #$to="To: john.mund\@noaa.gov";
   #$cc="";

   $sendmail = "/usr/sbin/sendmail -t -f john.mund\@noaa.gov -F DataRequest ";
...
sub MakeEmailContent()
{
   ...
   if ( $requestinfo )
   {
      ...
      $content = "${content}NOTE: The data file is being copied to the public data server.  This may take up to 30 minutes after the email has been sent.\n\n";
      $content = "${content}#######################################################\n";
   }
   ...
   else
   {
      ...
      $content = "${content}https://gml.noaa.gov/aftp/data/ccgg/requests/${outfile}\n\n"; 
