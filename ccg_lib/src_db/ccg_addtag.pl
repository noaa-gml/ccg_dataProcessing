...
#If $forceUpdate passed and we are in merge mode 0, remove any automated tags/flags before processing below.  This
#allows us to reprocess data but not clobber existing manual entries.  For tagged rows, we'll have to loop through and remove them.
#For regular flagged entries, we'll alter the update logic below to allow overwrite of automated flags as appropriate.
if($forceUpdate && $mergeMode==0){
	#See if there are existing automated tags for this data row.  Note we used to restrict to single row ranges but the view logic changed and that's
	#no longer convienent.  We'll assume that automated entries are always single row (because they are).
	my $sql="select r.range_num 
		from tag_range_info_view r, flask_data_tag_range d
 		where r.range_num=d.range_num and d.data_num=? and r.automated=1";#and r.is_data_range=1 and r.rowcount=1
	my $del=q{call tag_deleteTagRange(48,?,@v_status,@v_mssg,@v_numrows)};

	my $a=dosql($sql,($data_num));
	foreach my $row (@{$a}){#Remove any found.  They'll be re-added below if appropriate.
		my $range_num=${$row}{range_num};
		if($update){
			dodml($del,($range_num));
			if($verbose){print "Removed range:$range_num\n";}
		}else{print "Found range to remove:$range_num\n";}
...
if($needTag && !$tag_num && $flag ne '...'){
	...
	#If caller (data_source) was an automated script, filter available tags to automatic ones.  This helps with some name space collision.
	my $automatic=($data_source==14)?"and t.automated=1":"and t.automated=0";#jwm - 2/20/18. added auto=0 filter to make some flag selections work when both auto and non versions exist.  hopefully this doesn't cause errors with any automated callers (which should all be using data_source 14
	...
	my $sql2b="     lower(t.internal_flag)=? ";
	my $sql3="	and (t.program_num is null or t.program_num=d.program_num)
			...
			and (t.project_num is null or t.project_num=d.project_num) $automatic";
	$sql=$sql1.$sql2.$sql3;
	...
	foreach my $tflag (@flags){
		...
		if($tflag ne '...'){	
			...
			if($i==0){
				my $tflag2=lc($tflag);
				$a=dosql($sql1.$sql2b.$sql3 ,($data_num,$tflag2));
				$i=@{$a};
			}
...
if($needFlag){
	...
	#Note that merge mode 0 was updated 9/20/16 to include * first col after discussions with pat and kirk.
	#jwm 8/18 - updated again to overwrite any automated flag when forceupdate=1
	my $overrideFlags_rej="'.'";my $overrideFlags_inf="'.'";
	if($forceUpdate){
		#Fetch all automated ccgg measurement flags for rej and inf cols(note this generally (currently) includes *,<,>)
		my $sql="select group_concat(concat(\"'\",flag,\"'\")) from tag_view v where  (program_num is null or program_num=1) and automated=1 and ";
		my $t=dosqlval($sql."reject=1",());#first col reject flags
		if($t){$overrideFlags_rej.=",${t}";}
		$t=dosqlval($sql."information=1",());#3rd col info
		if($t){$overrideFlags_inf.=",${t}";}
	}else{#Standard logic overrides *.. and ..</>, append on.
		$overrideFlags_rej.=",'*'";
		$overrideFlags_inf.=",'>','<'";
	}
	my ($new1,$new2,$new3)=split(//,$flag);
	$sql="update flask_data d
		...
		when 0 then #automatic flagging scripts merge mode logic
			concat(
				case when substring(d.flag,1,1) in ($overrideFlags_rej) and '$new1' != '.' then '$new1' else substring(d.flag,1,1) end,
				case when substring(d.flag,2,1)='.' then '$new2' else substring(d.flag,2,1) end,
				case when substring(d.flag,3,1) in ($overrideFlags_inf) and '$new3' != '.' then '$new3' else substring(d.flag,3,1) end
		      	)
...
if($needTag && @tag_nums){
	...
	dodml("drop temporary table if exists t_data_nums");dodml("drop temporary table if exists t_event_nums");
...
sub doError(){
        my($msg)=@_;
        $msg="\n$msg\n\nParameters; data_num:$data_num tag_num:$tag_num flag:$flag event_num:$event_num data_source:$data_source parameter:$parameter adate:$adate atime:$atime inst:$inst mergeMode:$mergeMode.\n";
	#log error too.
	dodml(q{insert tag_entry_errors (data_num,flag,user,comment) select ?,?,user(),?},($data_num,$flag,$msg));

	#Email admin
	sendEmail('john.mund@noaa.gov','','','john.mund@noaa.gov','ccg_addFlag.pl error',$msg);
...
-mergeMode(default 0)
	...
	-mode=0 (is what is used on the automated flagging scripts).  Note; see below -forceUpdate switch for variations on this.
		...
		Note, that this has been refined to only update when new character is not '.' This is needed because we now apply 
		all automated flags as they come up (so tagging system rows can record them all)
...
-forceUpdate (default 0)
	...
	When passed 1 and mergeMode=0, we will remove existing automated tags and override any automated flags
